...
public class BufferBuilder extends DefaultedVertexConsumer implements BufferVertexConsumer {
    ...
    public void begin(VertexFormat.Mode p_166780_, VertexFormat p_166781_) {
        if (this.building) {
            ...
            this.currentElement = (VertexFormatElement)p_166781_.getElements().get(0);
            this.elementIndex = 0;
            this.buffer.rewind();
        }
    }
    ...
    private IntConsumer intConsumer(int p_231159_, VertexFormat.IndexType p_231160_) {
        ...
        switch (p_231160_) {
            case SHORT -> intconsumer = (p_231167_) -> {
    this.buffer.putShort(mutableint.getAndAdd(2), (short)p_231167_);
};
            case INT -> intconsumer = (p_231163_) -> {
    ...
    public RenderedBuffer endOrDiscardIfEmpty() {
        ...
        if (this.isCurrentBatchEmpty()) {
            ...
            RenderedBuffer bufferbuilder$renderedbuffer = this.storeRenderedBuffer();
            this.reset();
    ...
    private RenderedBuffer storeRenderedBuffer() {
        ...
        VertexFormat.IndexType vertexformat$indextype = IndexType.least(i);
        boolean flag;
        int k;
        int i1;
        if (this.sortingPoints != null) {
            i1 = Mth.roundToward(i * vertexformat$indextype.bytes, 4);
            this.ensureCapacity(i1);
            this.putSortedQuadIndices(vertexformat$indextype);
            flag = false;
            this.nextElementByte += i1;
            k = j + i1;
        } else {
            flag = true;
            k = j;
        ...
        i1 = this.renderedBufferPointer;
        this.renderedBufferPointer += k;
        ++this.renderedBufferCount;
        DrawState bufferbuilder$drawstate = new DrawState(this.format, this.vertices, i, this.mode, vertexformat$indextype, this.indexOnly, flag);
        return new RenderedBuffer(i1, bufferbuilder$drawstate);
    }
    ...
    public void nextElement() {
        ...
        if (vertexformatelement.getUsage() == Usage.PADDING) {
            this.nextElement();
        }

        if (this.defaultColorSet && this.currentElement.getUsage() == Usage.COLOR) {
