...
public class PoseStack implements IForgePoseStack {
    private final Deque<Pose> poseStack = (Deque)Util.make(Queues.newArrayDeque(), (p_85848_) -> {
        Matrix4f matrix4f = new Matrix4f();
        Matrix3f matrix3f = new Matrix3f();
        p_85848_.add(new Pose(matrix4f, matrix3f));
    });
    ...
    public void pushPose() {
        Pose posestack$pose = (Pose)this.poseStack.getLast();
        this.poseStack.addLast(new Pose(new Matrix4f(posestack$pose.pose), new Matrix3f(posestack$pose.normal)));
    }
    ...
    public void mulPoseMatrix(Matrix4f p_254128_) {
        ((Pose)this.poseStack.getLast()).pose.mul(p_254128_);
    }
