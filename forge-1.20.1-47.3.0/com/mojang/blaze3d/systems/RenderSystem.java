...
public class RenderSystem {
    ...
    private static final AutoStorageIndexBuffer sharedSequentialQuad = new AutoStorageIndexBuffer(4, 6, (p_157398_, p_157399_) -> {
    ...
    private static final AutoStorageIndexBuffer sharedSequentialLines = new AutoStorageIndexBuffer(4, 6, (p_157401_, p_157402_) -> {
    ...
    public static int maxSupportedTextureSize() {
        if (MAX_SUPPORTED_TEXTURE_SIZE == -1) {
            ...
            for(int $$1 = Math.max(32768, $$0); $$1 >= 1024; $$1 >>= 1) {
                GlStateManager._texImage2D(32868, 0, 6408, $$1, $$1, 0, 6408, 5121, (IntBuffer)null);
    ...
    public static void glBindVertexArray(Supplier<Integer> p_157212_) {
        GlStateManager._glBindVertexArray((Integer)p_157212_.get());
    }
    ...
    public static void _setShaderTexture(int p_157180_, ResourceLocation p_157181_) {
        if (p_157180_ >= 0 && p_157180_ < shaderTextures.length) {
            ...
            AbstractTexture $$3 = $$2.getTexture(p_157181_);
            shaderTextures[p_157180_] = $$3.getId();
        }
    ...
    public static void setProjectionMatrix(Matrix4f p_277884_, VertexSorting p_277702_) {
        Matrix4f $$2 = new Matrix4f(p_277884_);
        if (!isOnRenderThread()) {
            recordRenderCall(() -> {
                projectionMatrix = $$2;
    ...
    public static void setInverseViewRotationMatrix(Matrix3f p_253989_) {
        Matrix3f $$1 = new Matrix3f(p_253989_);
        if (!isOnRenderThread()) {
            recordRenderCall(() -> {
    ...
    public static void setTextureMatrix(Matrix4f p_254081_) {
        Matrix4f $$1 = new Matrix4f(p_254081_);
        if (!isOnRenderThread()) {
            recordRenderCall(() -> {
                textureMatrix = $$1;
            });
    ...
    public static final class AutoStorageIndexBuffer {
        ...
        private void ensureStorage(int p_157477_) {
            if (!this.hasStorage(p_157477_)) {
                ...
                VertexFormat.IndexType $$1 = IndexType.least(p_157477_);
                int $$2 = Mth.roundToward(p_157477_ * $$1.bytes, 4);
                ...
                if ($$3 == null) {
                    ...
                    for(int $$5 = 0; $$5 < p_157477_; $$5 += this.indexStride) {
        ...
        private it.unimi.dsi.fastutil.ints.IntConsumer intConsumer(ByteBuffer p_157479_) {
            switch (this.type) {
                ...
                case INT:
                default:
                    Objects.requireNonNull(p_157479_);
