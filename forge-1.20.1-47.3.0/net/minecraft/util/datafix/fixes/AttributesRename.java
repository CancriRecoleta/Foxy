...
public class AttributesRename extends DataFix {
    ...
    protected TypeRewriteRule makeRule() {
        ...
        return TypeRewriteRule.seq(this.fixTypeEverywhereTyped("Rename ItemStack Attributes", $$0, (p_14674_) -> {
            return p_14674_.updateTyped($$1, AttributesRename::fixItemStackTag);
    ...
    private static Dynamic<?> fixName(Dynamic<?> p_14678_) {
        Optional var10000 = p_14678_.asString().result().map((p_14680_) -> {
            return (String)RENAMES.getOrDefault(p_14680_, p_14680_);
        });
        Objects.requireNonNull(p_14678_);
        return (Dynamic)DataFixUtils.orElse(var10000.map(p_14678_::createString), p_14678_);
    }
    ...
    private static Typed<?> fixItemStackTag(Typed<?> p_14676_) {
        return p_14676_.update(DSL.remainderFinder(), (p_14694_) -> {
            return p_14694_.update("AttributeModifiers", (p_145080_) -> {
                Optional var10000 = p_145080_.asStreamOpt().result().map((p_145074_) -> {
                    return p_145074_.map((p_145082_) -> {
                        return p_145082_.update("AttributeName", AttributesRename::fixName);
                    });
                });
                Objects.requireNonNull(p_145080_);
                return (Dynamic)DataFixUtils.orElse(var10000.map(p_145080_::createList), p_145080_);
            });
        });
    }
    ...
    private static Typed<?> fixEntity(Typed<?> p_14684_) {
        return p_14684_.update(DSL.remainderFinder(), (p_14686_) -> {
            return p_14686_.update("Attributes", (p_145076_) -> {
                Optional var10000 = p_145076_.asStreamOpt().result().map((p_145072_) -> {
                    return p_145072_.map((p_145078_) -> {
                        return p_145078_.update("Name", AttributesRename::fixName);
                    });
                });
                Objects.requireNonNull(p_145076_);
                return (Dynamic)DataFixUtils.orElse(var10000.map(p_145076_::createList), p_145076_);
            });
        });
    }
}
