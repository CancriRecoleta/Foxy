    ...
    protected Typed<?> fix(Typed<?> p_15881_) {
        return p_15881_.update(DSL.remainderFinder(), (p_15883_) -> {
            return p_15883_.update("Gossips", (p_145376_) -> {
                Optional var10000 = p_145376_.asStreamOpt().result().map((p_145374_) -> {
                    return p_145374_.map((p_145378_) -> {
                        return (Dynamic)AbstractUUIDFix.replaceUUIDLeastMost(p_145378_, "Target", "Target").orElse(p_145378_);
                    });
                });
                Objects.requireNonNull(p_145376_);
                return (Dynamic)DataFixUtils.orElse(var10000.map(p_145376_::createList), p_145376_);
            });
        });
    }
}
