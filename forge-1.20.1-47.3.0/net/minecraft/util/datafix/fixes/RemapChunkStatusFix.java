...
public class RemapChunkStatusFix extends DataFix {
    ...
    private <T> Dynamic<T> fixStatus(Dynamic<T> p_281410_) {
        Optional var10000 = p_281410_.asString().result().map(NamespacedSchema::ensureNamespaced).map(this.mapper);
        Objects.requireNonNull(p_281410_);
        Optional<Dynamic<T>> $$1 = var10000.map(p_281410_::createString);
        return (Dynamic)DataFixUtils.orElse($$1, p_281410_);
    }
}
