    ...
    protected <T> Stream<Dynamic<T>> processRecords(Stream<Dynamic<T>> p_216716_) {
        return p_216716_.map((p_216714_) -> {
            return p_216714_.update("type", (p_216718_) -> {
                DataResult var10000 = p_216718_.asString().map(this.renamer);
                Objects.requireNonNull(p_216718_);
                return (Dynamic)DataFixUtils.orElse(var10000.map(p_216718_::createString).result(), p_216718_);
            });
        });
    }
}
