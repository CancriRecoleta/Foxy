                ...
                while(true) {
                    ...
                    while(true) {
                        ...
                        do {
                            do {
                                if (!var6.hasNext()) {
                                    return (Typed)((Pair)$$0.readTyped($$2.emptyMap().set("stats", $$2.createMap($$3))).result().orElseThrow(() -> {
                                        return new IllegalStateException("Could not parse new stats object.");
                                    })).getFirst();
                                }

                                $$5 = (Map.Entry)var6.next();
                            } while(!((Dynamic)$$5.getValue()).asNumber().result().isPresent());

                            $$6 = ((Dynamic)$$5.getKey()).asString("");
                        } while(SKIP.contains($$6));

                        if (CUSTOM_MAP.containsKey($$6)) {
                            $$17 = "minecraft:custom";
                            $$19 = (String)CUSTOM_MAP.get($$6);
                            break;
                        }
                        ...
                        if ($$9 >= 0) {
                            ...
                            if ("stat.mineBlock".equals($$10)) {
                                $$17 = "minecraft:mined";
                                $$19 = this.upgradeBlock($$6.substring($$9 + 1).replace('.', ':'));
                                break;
                            }

                            String $$18;
                            if (ITEM_KEYS.containsKey($$10)) {
                                $$17 = (String)ITEM_KEYS.get($$10);
                                $$18 = $$6.substring($$9 + 1).replace('.', ':');
                                String $$15 = this.upgradeItem($$18);
                                $$19 = $$15 == null ? $$18 : $$15;
                                break;
                            }

                            if (ENTITY_KEYS.containsKey($$10)) {
                                $$17 = (String)ENTITY_KEYS.get($$10);
                                $$18 = $$6.substring($$9 + 1).replace('.', ':');
                                $$19 = (String)ENTITIES.getOrDefault($$18, $$18);
                                break;
                            }
                        }
                    }
                    ...
                    Dynamic<?> $$21 = (Dynamic)$$3.computeIfAbsent($$20, (p_145701_) -> {
                        return $$2.emptyMap();
                    });
