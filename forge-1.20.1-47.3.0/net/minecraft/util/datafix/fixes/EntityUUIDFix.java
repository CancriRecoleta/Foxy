...
public class EntityUUIDFix extends AbstractUUIDFix {
    ...
    protected TypeRewriteRule makeRule() {
        return this.fixTypeEverywhereTyped("EntityUUIDFixes", this.getInputSchema().getType(this.typeReference), (p_15725_) -> {
            ...
            Iterator var2;
            String $$6;
            for(var2 = ABSTRACT_HORSES.iterator(); var2.hasNext(); p_15725_ = this.updateNamedChoice(p_15725_, $$6, EntityUUIDFix::updateAnimalOwner)) {
                $$6 = (String)var2.next();
            }

            for(var2 = TAMEABLE_ANIMALS.iterator(); var2.hasNext(); p_15725_ = this.updateNamedChoice(p_15725_, $$6, EntityUUIDFix::updateAnimalOwner)) {
                $$6 = (String)var2.next();
            }

            for(var2 = ANIMALS.iterator(); var2.hasNext(); p_15725_ = this.updateNamedChoice(p_15725_, $$6, EntityUUIDFix::updateAnimal)) {
                $$6 = (String)var2.next();
            }

            for(var2 = MOBS.iterator(); var2.hasNext(); p_15725_ = this.updateNamedChoice(p_15725_, $$6, EntityUUIDFix::updateMob)) {
                $$6 = (String)var2.next();
            }

            for(var2 = LIVING_ENTITIES.iterator(); var2.hasNext(); p_15725_ = this.updateNamedChoice(p_15725_, $$6, EntityUUIDFix::updateLivingEntity)) {
                $$6 = (String)var2.next();
            }

            for(var2 = PROJECTILES.iterator(); var2.hasNext(); p_15725_ = this.updateNamedChoice(p_15725_, $$6, EntityUUIDFix::updateProjectile)) {
                $$6 = (String)var2.next();
            }
            ...
            p_15725_ = this.updateNamedChoice(p_15725_, "minecraft:piglin", EntityUUIDFix::updatePiglin);
            return p_15725_;
        });
    }
        ...
        return p_15740_.update("Brain", (p_15781_) -> {
            return p_15781_.update("memories", (p_145345_) -> {
                return p_145345_.update("minecraft:angry_at", (p_145347_) -> {
                    return (Dynamic)replaceUUIDString(p_145347_, "value", "value").orElseGet(() -> {
    ...
    private static Dynamic<?> updateEvokerFangs(Dynamic<?> p_15745_) {
        return (Dynamic)replaceUUIDLeastMost(p_15745_, "OwnerUUID", "Owner").orElse(p_15745_);
    }

    private static Dynamic<?> updateZombieVillager(Dynamic<?> p_15750_) {
        return (Dynamic)replaceUUIDLeastMost(p_15750_, "ConversionPlayer", "ConversionPlayer").orElse(p_15750_);
    }

    private static Dynamic<?> updateAreaEffectCloud(Dynamic<?> p_15752_) {
        return (Dynamic)replaceUUIDLeastMost(p_15752_, "OwnerUUID", "Owner").orElse(p_15752_);
    }

    private static Dynamic<?> updateShulkerBullet(Dynamic<?> p_15754_) {
        p_15754_ = (Dynamic)replaceUUIDMLTag(p_15754_, "Owner", "Owner").orElse(p_15754_);
        return (Dynamic)replaceUUIDMLTag(p_15754_, "Target", "Target").orElse(p_15754_);
    }

    private static Dynamic<?> updateItem(Dynamic<?> p_15756_) {
        p_15756_ = (Dynamic)replaceUUIDMLTag(p_15756_, "Owner", "Owner").orElse(p_15756_);
        return (Dynamic)replaceUUIDMLTag(p_15756_, "Thrower", "Thrower").orElse(p_15756_);
    }

    private static Dynamic<?> updateFox(Dynamic<?> p_15758_) {
        Optional<Dynamic<?>> $$1 = p_15758_.get("TrustedUUIDs").result().map((p_15748_) -> {
            return p_15758_.createList(p_15748_.asStream().map((p_145341_) -> {
                return (Dynamic)createUUIDFromML(p_145341_).orElseGet(() -> {
                    LOGGER.warn("Trusted contained invalid data.");
                    return p_145341_;
                });
            }));
        });
        return (Dynamic)DataFixUtils.orElse($$1.map((p_15743_) -> {
            return p_15758_.remove("TrustedUUIDs").set("Trusted", p_15743_);
        }), p_15758_);
    }

    private static Dynamic<?> updateHurtBy(Dynamic<?> p_15760_) {
        return (Dynamic)replaceUUIDString(p_15760_, "HurtBy", "HurtBy").orElse(p_15760_);
    }

    private static Dynamic<?> updateAnimalOwner(Dynamic<?> p_15762_) {
        Dynamic<?> $$1 = updateAnimal(p_15762_);
        return (Dynamic)replaceUUIDString($$1, "OwnerUUID", "Owner").orElse($$1);
    }

    private static Dynamic<?> updateAnimal(Dynamic<?> p_15764_) {
        Dynamic<?> $$1 = updateMob(p_15764_);
    ...
    private static Dynamic<?> updateMob(Dynamic<?> p_15767_) {
        return updateLivingEntity(p_15767_).update("Leash", (p_15775_) -> {
            return (Dynamic)replaceUUIDLeastMost(p_15775_, "UUID", "UUID").orElse(p_15775_);
        });
    }
            ...
            return p_15730_.createList(p_15733_.asStream().map((p_145337_) -> {
                return p_145337_.update("Modifiers", (p_145335_) -> {
                    return p_145337_.createList(p_145335_.asStream().map((p_145339_) -> {
                        return (Dynamic)replaceUUIDLeastMost(p_145339_, "UUID", "UUID").orElse(p_145339_);
                    }));
                });
            }));
        });
    ...
    private static Dynamic<?> updateProjectile(Dynamic<?> p_15769_) {
        return (Dynamic)DataFixUtils.orElse(p_15769_.get("OwnerUUID").result().map((p_15728_) -> {
            return p_15769_.remove("OwnerUUID").set("Owner", p_15728_);
        }), p_15769_);
    }

    public static Dynamic<?> updateEntityUUID(Dynamic<?> p_15735_) {
        return (Dynamic)replaceUUIDLeastMost(p_15735_, "UUID", "UUID").orElse(p_15735_);
    }
