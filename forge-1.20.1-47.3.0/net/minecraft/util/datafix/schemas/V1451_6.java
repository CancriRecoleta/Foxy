...
public class V1451_6 extends NamespacedSchema {
    ...
    protected static final Hook.HookFunction UNPACK_OBJECTIVE_ID = new Hook.HookFunction() {
        public <T> T apply(DynamicOps<T> p_181096_, T p_181097_) {
            ...
            return ((Dynamic)DataFixUtils.orElse($$2.get("CriteriaName").asString().get().left().map((p_181094_) -> {
                int $$1 = p_181094_.indexOf(58);
                if ($$1 < 0) {
                    return Pair.of("_special", p_181094_);
                } else {
                    try {
                        ResourceLocation $$2 = ResourceLocation.of(p_181094_.substring(0, $$1), '.');
                ...
                return $$2.set("CriteriaType", $$2.createMap(ImmutableMap.of($$2.createString("type"), $$2.createString((String)p_181092_.getFirst()), $$2.createString("id"), $$2.createString((String)p_181092_.getSecond()))));
            }), $$2)).getValue();
        }
    };
    protected static final Hook.HookFunction REPACK_OBJECTIVE_ID = new Hook.HookFunction() {
        ...
        public <T> T apply(DynamicOps<T> p_181105_, T p_181106_) {
            ...
            Optional<Dynamic<T>> $$3 = $$2.get("CriteriaType").get().get().left().flatMap((p_181109_) -> {
                ...
                Optional<String> $$3 = p_181109_.get("id").asString().get().left();
                if ($$2x.isPresent() && $$3.isPresent()) {
                    String $$4 = (String)$$2x.get();
                    if ($$4.equals("_special")) {
                        return Optional.of($$2.createString((String)$$3.get()));
                    } else {
                        String var10001 = this.packWithDot($$4);
                        return Optional.of(p_181109_.createString(var10001 + ":" + this.packWithDot((String)$$3.get())));
                    }
                } else {
                    return Optional.empty();
                }
            });
            return ((Dynamic)DataFixUtils.orElse($$3.map((p_181101_) -> {
                return $$2.set("CriteriaName", p_181101_).remove("CriteriaType");
            }), $$2)).getValue();
        }
    };
    ...
    public void registerTypes(Schema p_17540_, Map<String, Supplier<TypeTemplate>> p_17541_, Map<String, Supplier<TypeTemplate>> p_17542_) {
        ...
        p_17540_.registerType(false, References.STATS, () -> {
            return DSL.optionalFields("stats", DSL.optionalFields("minecraft:mined", DSL.compoundList(References.BLOCK_NAME.in(p_17540_), DSL.constType(DSL.intType())), "minecraft:crafted", (TypeTemplate)$$3.get(), "minecraft:used", (TypeTemplate)$$3.get(), "minecraft:broken", (TypeTemplate)$$3.get(), "minecraft:picked_up", (TypeTemplate)$$3.get(), DSL.optionalFields("minecraft:dropped", (TypeTemplate)$$3.get(), "minecraft:killed", DSL.compoundList(References.ENTITY_NAME.in(p_17540_), DSL.constType(DSL.intType())), "minecraft:killed_by", DSL.compoundList(References.ENTITY_NAME.in(p_17540_), DSL.constType(DSL.intType())), "minecraft:custom", DSL.compoundList(DSL.constType(namespacedString()), DSL.constType(DSL.intType())))));
        });
        Map<String, Supplier<TypeTemplate>> $$4 = createCriterionTypes(p_17540_);
        p_17540_.registerType(false, References.OBJECTIVE, () -> {
            return DSL.hook(DSL.optionalFields("CriteriaType", DSL.taggedChoiceLazy("type", DSL.string(), $$4)), UNPACK_OBJECTIVE_ID, REPACK_OBJECTIVE_ID);
        });
    }
