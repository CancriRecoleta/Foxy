...
public class SpawnUtil {
    ...
    public static <T extends Mob> Optional<T> trySpawnMob(EntityType<T> p_216404_, MobSpawnType p_216405_, ServerLevel p_216406_, BlockPos p_216407_, int p_216408_, int p_216409_, int p_216410_, Strategy p_216411_) {
        ...
        for(int i = 0; i < p_216408_; ++i) {
            ...
            if (p_216406_.getWorldBorder().isWithinBounds((BlockPos)blockpos$mutableblockpos) && moveToPossibleSpawnPosition(p_216406_, p_216410_, blockpos$mutableblockpos, p_216411_)) {
                T t = (Mob)p_216404_.create(p_216406_, (CompoundTag)null, (Consumer)null, blockpos$mutableblockpos, p_216405_, false, false);
                if (t != null) {
                    if (ForgeEventFactory.checkSpawnPosition(t, p_216406_, p_216405_)) {
    ...
    private static boolean moveToPossibleSpawnPosition(ServerLevel p_216399_, int p_216400_, BlockPos.MutableBlockPos p_216401_, Strategy p_216402_) {
        ...
        for(int i = p_216400_; i >= -p_216400_; --i) {
            ...
            blockpos$mutableblockpos.setWithOffset(p_216401_, (Direction)Direction.UP);
    ...
    public interface Strategy {
        /** @deprecated */
        @Deprecated
        Strategy LEGACY_IRON_GOLEM = (p_289751_, p_289752_, p_289753_, p_289754_, p_289755_) -> {
        ...
        Strategy ON_TOP_OF_COLLIDER = (p_216416_, p_216417_, p_216418_, p_216419_, p_216420_) -> {
