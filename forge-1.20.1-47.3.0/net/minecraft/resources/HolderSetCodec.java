...
public class HolderSetCodec<E> implements Codec<HolderSet<E>> {
    ...
    private static <E> Codec<List<Holder<E>>> homogenousList(Codec<Holder<E>> p_206668_, boolean p_206669_) {
        ...
        return p_206669_ ? codec : Codec.either(codec, p_206668_).xmap((p_206664_) -> {
            return (List)p_206664_.map((p_206694_) -> {
                return p_206694_;
            }, List::of);
        }, (p_206684_) -> {
            return p_206684_.size() == 1 ? Either.right((Holder)p_206684_.get(0)) : Either.left(p_206684_);
        });
    }
    ...
    private HolderSetCodec(ResourceKey<? extends Registry<E>> p_206660_, Codec<Holder<E>> p_206661_, boolean p_206662_) {
        ...
        this.forgeDispatchCodec = ExtraCodecs.lazyInitializedCodec(() -> {
            return ((IForgeRegistry)ForgeRegistries.HOLDER_SET_TYPES.get()).getCodec();
        }).dispatch(ICustomHolderSet::type, (type) -> {
            return type.makeCodec(p_206660_, p_206661_, p_206662_);
        });
            ...
            if (optional.isPresent()) {
                ...
                return this.combinedCodec.decode(p_206696_, p_206697_).map((p_206682_) -> {
                    return p_206682_.mapFirst((p_206679_) -> {
                        return (HolderSet)p_206679_.map(Function.identity(), (tagOrList) -> {
                            Objects.requireNonNull(holdergetter);
                            return (HolderSet)tagOrList.map(holdergetter::getOrThrow, HolderSet::direct);
                        });
                    });
                });
            }
    ...
    public <T> DataResult<T> encode(HolderSet<E> p_206674_, DynamicOps<T> p_206675_, T p_206676_) {
        if (p_206675_ instanceof RegistryOps<T> registryops) {
            ...
            if (optional.isPresent()) {
                if (!p_206674_.canSerializeIn((HolderOwner)optional.get())) {
                ...
                if (p_206674_ instanceof ICustomHolderSet) {
                    ICustomHolderSet<E> customHolderSet = (ICustomHolderSet)p_206674_;
    ...
    private <T> DataResult<Pair<HolderSet<E>, T>> decodeWithoutRegistry(DynamicOps<T> p_206671_, T p_206672_) {
        return this.elementCodec.listOf().decode(p_206671_, p_206672_).flatMap((p_206666_) -> {
            ...
            Iterator var2 = ((List)p_206666_.getFirst()).iterator();

            while(var2.hasNext()) {
                Holder<E> holder = (Holder)var2.next();
            ...
            return DataResult.success(new Pair(HolderSet.direct((List)list), p_206666_.getSecond()));
        });
    }
