...
public final class RegistryFixedCodec<E> implements Codec<Holder<E>> {
    ...
    public <T> DataResult<T> encode(Holder<E> p_206729_, DynamicOps<T> p_206730_, T p_206731_) {
        if (p_206730_ instanceof RegistryOps<?> $$3) {
            Optional<HolderOwner<E>> $$4 = $$3.owner(this.registryKey);
            if ($$4.isPresent()) {
                if (!p_206729_.canSerializeIn((HolderOwner)$$4.get())) {
                    return DataResult.error(() -> {
                        return "Element " + p_206729_ + " is not valid in current registry set";
                    });
                }

                return (DataResult)p_206729_.unwrap().map((p_206727_) -> {
    ...
    public <T> DataResult<Pair<Holder<E>, T>> decode(DynamicOps<T> p_206743_, T p_206744_) {
        if (p_206743_ instanceof RegistryOps<?> $$2) {
            ...
            if ($$3.isPresent()) {
                return ResourceLocation.CODEC.decode(p_206743_, p_206744_).flatMap((p_255515_) -> {
                    ResourceLocation $$2 = (ResourceLocation)p_255515_.getFirst();
                    return ((DataResult)((HolderGetter)$$3.get()).get(ResourceKey.create(this.registryKey, $$2)).map(DataResult::success).orElseGet(() -> {
                        return DataResult.error(() -> {
                            return "Failed to get element " + $$2;
                        });
                    })).map((p_256041_) -> {
                        return Pair.of(p_256041_, p_255515_.getSecond());
