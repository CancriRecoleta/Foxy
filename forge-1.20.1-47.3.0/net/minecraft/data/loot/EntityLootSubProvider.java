...
public abstract class EntityLootSubProvider implements LootTableSubProvider {
    ...
    public void generate(BiConsumer<ResourceLocation, LootTable.Builder> p_251751_) {
        ...
        this.getKnownEntityTypes().map(EntityType::builtInRegistryHolder).forEach((p_266624_) -> {
            EntityType<?> entitytype = (EntityType)p_266624_.value();
            if (entitytype.isEnabled(this.allowed)) {
                ...
                if (this.canHaveLootTable(entitytype)) {
                    ...
                    map = (Map)this.map.remove(entitytype);
                    if (map != null) {
                        throw new IllegalStateException(String.format(Locale.ROOT, "Weird loottables '%s' for '%s', not a LivingEntity so should not have loot", map.keySet().stream().map(ResourceLocation::toString).collect(Collectors.joining(",")), p_266624_.key().location()));
                    }
                }
            }
    ...
    protected LootItemCondition.Builder killedByFrogVariant(FrogVariant p_249403_) {
        return DamageSourceCondition.hasDamageSource(net.minecraft.advancements.critereon.DamageSourcePredicate.Builder.damageType().source(Builder.entity().of(EntityType.FROG).subPredicate(EntitySubPredicate.variant(p_249403_))));
    }
    ...
    protected void add(EntityType<?> p_252130_, ResourceLocation p_251706_, LootTable.Builder p_249357_) {
        ((Map)this.map.computeIfAbsent(p_252130_, (p_251466_) -> {
            return new HashMap();
