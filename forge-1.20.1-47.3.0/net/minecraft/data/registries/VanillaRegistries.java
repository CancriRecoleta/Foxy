...
public class VanillaRegistries {
    ...
    public static void validateThatAllBiomeFeaturesHaveBiomeFilter(HolderGetter<PlacedFeature> p_272963_, HolderLookup<Biome> p_273693_) {
        p_273693_.listElements().forEach((p_256326_) -> {
            ...
            List<HolderSet<PlacedFeature>> list = ((Biome)p_256326_.value()).getGenerationSettings().features();
            list.stream().flatMap(HolderSet::stream).forEach((p_256657_) -> {
                p_256657_.unwrap().ifLeft((p_256188_) -> {
                    ...
                    if (!validatePlacedFeature((PlacedFeature)reference.value())) {
                        ResourceLocation var10000 = p_256188_.location();
                        Util.logAndPauseIfInIde("Placed feature " + var10000 + " in biome " + resourcelocation + " is missing BiomeFilter.biome()");
                    }
