...
public class Main {
    ...
    public static void main(String[] p_129669_) throws IOException {
        ...
        if (!optionset.has(optionspec) && optionset.hasOptions() && (optionset.specs().size() != 1 || !optionset.has(gameDir))) {
            ...
            Collection<Path> inputs = (Collection)optionset.valuesOf(optionspec8).stream().map((p_129659_) -> {
                return Paths.get(p_129659_);
            }).collect(Collectors.toList());
            Collection<Path> existingPacks = (Collection)optionset.valuesOf(existing).stream().map((x$0) -> {
                return Paths.get(x$0);
            }).collect(Collectors.toList());
            Set<String> existingMods = new HashSet(optionset.valuesOf(existingMod));
            Set<String> mods = new HashSet(optionset.valuesOf(mod));
            boolean isFlat = mods.isEmpty() || optionset.has(flat);
            DatagenModLoader.begin(mods, path, inputs, existingPacks, existingMods, flag2, flag1, flag3, flag4, flag5, isFlat, (String)optionset.valueOf(assetIndex), (File)optionset.valueOf(assetsDir));
    ...
    private static <T extends DataProvider> DataProvider.Factory<T> bindRegistries(BiFunction<PackOutput, CompletableFuture<HolderLookup.Provider>, T> p_256618_, CompletableFuture<HolderLookup.Provider> p_256515_) {
        return (p_255476_) -> {
            return (DataProvider)p_256618_.apply(p_255476_, p_256515_);
        };
    }
    ...
    public static DataGenerator createStandardGenerator(Path p_236680_, Collection<Path> p_236681_, boolean p_236682_, boolean p_236683_, boolean p_236684_, boolean p_236685_, boolean p_236686_, WorldVersion p_236687_, boolean p_236688_) {
        ...
        TagsProvider<Block> tagsprovider1 = (TagsProvider)datagenerator$packgenerator2.addProvider(bindRegistries(VanillaBlockTagsProvider::new, completablefuture));
        TagsProvider<Item> tagsprovider = (TagsProvider)datagenerator$packgenerator2.addProvider((p_274753_) -> {
