...
public class RegistryCodecs {
    ...
    private static <T> MapCodec<RegistryEntry<T>> withNameAndId(ResourceKey<? extends Registry<T>> p_206304_, MapCodec<T> p_206305_) {
        return RecordCodecBuilder.mapCodec((p_206309_) -> {
            return p_206309_.group(ResourceKey.codec(p_206304_).fieldOf("name").forGetter(RegistryEntry::key), Codec.INT.fieldOf("id").forGetter(RegistryEntry::id), p_206305_.forGetter(RegistryEntry::value)).apply(p_206309_, RegistryEntry::new);
        });
    }
    ...
    public static <T> Codec<Registry<T>> networkCodec(ResourceKey<? extends Registry<T>> p_206292_, Lifecycle p_206293_, Codec<T> p_206294_) {
        return withNameAndId(p_206292_, p_206294_.fieldOf("element")).codec().listOf().xmap((p_258188_) -> {
            ...
            Iterator var4 = p_258188_.iterator();

            while(var4.hasNext()) {
                RegistryEntry<T> registryentry = (RegistryEntry)var4.next();
            ...
            Iterator var2 = p_258185_.iterator();

            while(var2.hasNext()) {
                T t = var2.next();
                builder.add(new RegistryEntry((ResourceKey)p_258185_.getResourceKey(t).get(), p_258185_.getId(t), t));
            }
    ...
    public static <E> Codec<Registry<E>> fullCodec(ResourceKey<? extends Registry<E>> p_248884_, Lifecycle p_251810_, Codec<E> p_250169_) {
        Codec<Map<ResourceKey<E>, E>> codec = new LenientUnboundedMapCodec(ResourceKey.codec(p_248884_), p_250169_);
    ...
    static record RegistryEntry<T>(ResourceKey<T> key, int id, T value) {
        RegistryEntry(ResourceKey<T> key, int id, T value) {
            this.key = key;
            this.id = id;
            this.value = value;
        }
