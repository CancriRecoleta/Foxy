...
public interface Registry<T> extends Keyable, IdMap<T> {
    ...
    default Codec<T> byNameCodec() {
        Codec<T> $$0 = ResourceLocation.CODEC.flatXmap((p_258170_) -> {
            return (DataResult)Optional.ofNullable(this.get(p_258170_)).map(DataResult::success).orElseGet(() -> {
                return DataResult.error(() -> {
                    ResourceKey var10000 = this.key();
                    return "Unknown registry key in " + var10000 + ": " + p_258170_;
                });
            });
        }, (p_258177_) -> {
            return (DataResult)this.getResourceKey(p_258177_).map(ResourceKey::location).map(DataResult::success).orElseGet(() -> {
                return DataResult.error(() -> {
                    ResourceKey var10000 = this.key();
                    return "Unknown registry element in " + var10000 + ":" + p_258177_;
                });
            });
        });
        Codec<T> $$1 = ExtraCodecs.idResolverCodec((p_258179_) -> {
            return this.getResourceKey(p_258179_).isPresent() ? this.getId(p_258179_) : -1;
        }, this::byId, -1);
        return ExtraCodecs.overrideLifecycle(ExtraCodecs.orCompressed($$0, $$1), this::lifecycle, this::lifecycle);
    }

    default Codec<Holder<T>> holderByNameCodec() {
        Codec<Holder<T>> $$0 = ResourceLocation.CODEC.flatXmap((p_258174_) -> {
            return (DataResult)this.getHolder(ResourceKey.create(this.key(), p_258174_)).map(DataResult::success).orElseGet(() -> {
                return DataResult.error(() -> {
                    ResourceKey var10000 = this.key();
                    return "Unknown registry key in " + var10000 + ": " + p_258174_;
                });
            });
        }, (p_206061_) -> {
            return (DataResult)p_206061_.unwrapKey().map(ResourceKey::location).map(DataResult::success).orElseGet(() -> {
                return DataResult.error(() -> {
                    ResourceKey var10000 = this.key();
                    return "Unknown registry element in " + var10000 + ":" + p_206061_;
                });
            });
        });
        return ExtraCodecs.overrideLifecycle($$0, (p_258178_) -> {
    ...
    default T getOrThrow(ResourceKey<T> p_123014_) {
        T $$1 = this.get(p_123014_);
        if ($$1 == null) {
            ResourceKey var10002 = this.key();
            throw new IllegalStateException("Missing key in " + var10002 + ": " + p_123014_);
        } else {
            return $$1;
        }
    }
    ...
    default Holder.Reference<T> getHolderOrThrow(ResourceKey<T> p_249087_) {
        return (Holder.Reference)this.getHolder(p_249087_).orElseThrow(() -> {
            ResourceKey var10002 = this.key();
            return new IllegalStateException("Missing key in " + var10002 + ": " + p_249087_);
        });
    }
    ...
    default IdMap<Holder<T>> asHolderIdMap() {
        return new IdMap<Holder<T>>() {
            ...
            public Holder<T> byId(int p_259972_) {
                return (Holder)Registry.this.getHolder(p_259972_).orElse((Object)null);
            }
