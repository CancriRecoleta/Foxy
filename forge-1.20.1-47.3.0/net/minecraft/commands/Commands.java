...
public class Commands {
    ...
    public Commands(CommandSelection p_230943_, CommandBuildContext p_230944_) {
        ...
        SeedCommand.register(this.dispatcher, p_230943_ != net.minecraft.commands.Commands.CommandSelection.INTEGRATED);
        ...
        ForgeEventFactory.onCommandRegister(this.dispatcher, p_230943_, p_230944_);
        this.dispatcher.setConsumer((p_230954_, p_230955_, p_230956_) -> {
            ((CommandSourceStack)p_230954_.getSource()).onCommandComplete(p_230954_, p_230955_, p_230956_);
            ((CommandSourceStack)p_230954_.getSource()).onCommandComplete(p_230954_, p_230955_, p_230956_);
        });
    }
    ...
    public int performCommand(ParseResults<CommandSourceStack> p_242844_, String p_242841_) {
        ...
        try {
            ...
            byte var22;
            try {
                CommandEvent event = new CommandEvent(p_242844_);
                if (!MinecraftForge.EVENT_BUS.post(event)) {
                    j = this.dispatcher.execute(event.getParseResults());
                    return j;
                }

                Throwable var26 = event.getException();
                if (var26 instanceof Exception exception) {
                    throw exception;
                }
                ...
                if (var14.getInput() != null && var14.getCursor() >= 0) {
                    j = Math.min(var14.getInput().length(), var14.getCursor());
                    MutableComponent mutablecomponent1 = Component.empty().withStyle(ChatFormatting.GRAY).withStyle((p_82134_) -> {
                        return p_82134_.withClickEvent(new ClickEvent(Action.SUGGEST_COMMAND, "/" + p_242841_));
                    });
                    if (j > 10) {
                        mutablecomponent1.append(CommonComponents.ELLIPSIS);
                    }

                    mutablecomponent1.append(var14.getInput().substring(Math.max(0, j - 10), j));
                    if (j < var14.getInput().length()) {
                        Component component = Component.literal(var14.getInput().substring(j)).withStyle(ChatFormatting.RED, ChatFormatting.UNDERLINE);
                        mutablecomponent1.append((Component)component);
                    }

                    mutablecomponent1.append((Component)Component.translatable("command.context.here").withStyle(ChatFormatting.RED, ChatFormatting.ITALIC));
                ...
                commandsourcestack.sendFailure(Component.translatable("command.failed").withStyle((p_82137_) -> {
                    return p_82137_.withHoverEvent(new HoverEvent(net.minecraft.network.chat.HoverEvent.Action.SHOW_TEXT, mutablecomponent));
                }));
    ...
    public void sendCommands(ServerPlayer p_82096_) {
        ...
        CommandHelper.mergeCommandNode(this.dispatcher.getRoot(), rootcommandnode, map, p_82096_.createCommandSourceStack(), (ctx) -> {
            return 0;
        }, (suggest) -> {
            return SuggestionProviders.safelySwap(suggest);
        });
    ...
    private void fillUsableCommands(CommandNode<CommandSourceStack> p_82113_, CommandNode<SharedSuggestionProvider> p_82114_, CommandSourceStack p_82115_, Map<CommandNode<CommandSourceStack>, CommandNode<SharedSuggestionProvider>> p_82116_) {
        Iterator var5 = p_82113_.getChildren().iterator();

        while(var5.hasNext()) {
            CommandNode<CommandSourceStack> commandnode = (CommandNode)var5.next();
            if (commandnode.canUse(p_82115_)) {
                ...
                if (argumentbuilder.getRedirect() != null) {
                    argumentbuilder.redirect((CommandNode)p_82116_.get(argumentbuilder.getRedirect()));
                }
    ...
    public static void validate() {
        ...
        CommandDispatcher<CommandSourceStack> commanddispatcher = (new Commands(net.minecraft.commands.Commands.CommandSelection.ALL, commandbuildcontext)).getDispatcher();
        RootCommandNode<CommandSourceStack> rootcommandnode = commanddispatcher.getRoot();
        commanddispatcher.findAmbiguities((p_230947_, p_230948_, p_230949_, p_230950_) -> {
            LOGGER.warn("Ambiguity between arguments {} and {} with inputs: {}", new Object[]{commanddispatcher.getPath(p_230948_), commanddispatcher.getPath(p_230949_), p_230950_});
        });
        Set<ArgumentType<?>> set = ArgumentUtils.findUsedArgumentTypes(rootcommandnode);
        Set<ArgumentType<?>> set1 = (Set)set.stream().filter((p_230961_) -> {
