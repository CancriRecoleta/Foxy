...
public class SpreadPlayersCommand {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> p_138697_) {
        p_138697_.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("spreadplayers").requires((p_201852_) -> {
            return p_201852_.hasPermission(2);
        })).then(Commands.argument("center", Vec2Argument.vec2()).then(Commands.argument("spreadDistance", FloatArgumentType.floatArg(0.0F)).then(((RequiredArgumentBuilder)Commands.argument("maxRange", FloatArgumentType.floatArg(1.0F)).then(Commands.argument("respectTeams", BoolArgumentType.bool()).then(Commands.argument("targets", EntityArgument.entities()).executes((p_288627_) -> {
            return spreadPlayers((CommandSourceStack)p_288627_.getSource(), Vec2Argument.getVec2(p_288627_, "center"), FloatArgumentType.getFloat(p_288627_, "spreadDistance"), FloatArgumentType.getFloat(p_288627_, "maxRange"), ((CommandSourceStack)p_288627_.getSource()).getLevel().getMaxBuildHeight(), BoolArgumentType.getBool(p_288627_, "respectTeams"), EntityArgument.getEntities(p_288627_, "targets"));
    ...
    private static int spreadPlayers(CommandSourceStack p_138703_, Vec2 p_138704_, float p_138705_, float p_138706_, int p_138707_, boolean p_138708_, Collection<? extends Entity> p_138709_) throws CommandSyntaxException {
        ...
        if (p_138707_ < i) {
            ...
            Position[] aspreadplayerscommand$position = createInitialPositions(randomsource, p_138708_ ? getNumberOfTeams(p_138709_) : p_138709_.size(), d0, d1, d2, d3);
    ...
    private static int getNumberOfTeams(Collection<? extends Entity> p_138728_) {
        ...
        Iterator var2 = p_138728_.iterator();
    ...
    private static void spreadPositions(Vec2 p_214741_, double p_214742_, ServerLevel p_214743_, RandomSource p_214744_, double p_214745_, double p_214746_, double p_214747_, double p_214748_, int p_214749_, Position[] p_214750_, boolean p_214751_) throws CommandSyntaxException {
        ...
        for(i = 0; i < 10000 && flag; ++i) {
            ...
            int k;
            Position spreadplayerscommand$position3;
            for(int j = 0; j < p_214750_.length; ++j) {
                Position spreadplayerscommand$position = p_214750_[j];
                k = 0;
                spreadplayerscommand$position3 = new Position();

                for(int l = 0; l < p_214750_.length; ++l) {
                    if (j != l) {
                        Position spreadplayerscommand$position2 = p_214750_[l];
                        double d1 = spreadplayerscommand$position.dist(spreadplayerscommand$position2);
                        d0 = Math.min(d1, d0);
                        if (d1 < p_214742_) {
                            ++k;
                            spreadplayerscommand$position3.x += spreadplayerscommand$position2.x - spreadplayerscommand$position.x;
                            spreadplayerscommand$position3.z += spreadplayerscommand$position2.z - spreadplayerscommand$position.z;
                        }
                    }
                }

                if (k > 0) {
                    spreadplayerscommand$position3.x /= (double)k;
                    spreadplayerscommand$position3.z /= (double)k;
                    double d2 = spreadplayerscommand$position3.getLength();
                    if (d2 > 0.0) {
                        spreadplayerscommand$position3.normalize();
                        spreadplayerscommand$position.moveAway(spreadplayerscommand$position3);
                    } else {
            ...
            if (!flag) {
                Position[] var28 = p_214750_;
                int var29 = p_214750_.length;

                for(k = 0; k < var29; ++k) {
                    spreadplayerscommand$position3 = var28[k];
    ...
    private static double setPlayerPositions(Collection<? extends Entity> p_138730_, ServerLevel p_138731_, Position[] p_138732_, int p_138733_, boolean p_138734_) {
        ...
        double d2;
        for(Iterator var9 = p_138730_.iterator(); var9.hasNext(); d0 += d2) {
            ...
            EntityTeleportEvent.SpreadPlayersCommand event = ForgeEventFactory.onEntityTeleportSpreadPlayersCommand(entity, (double)Mth.floor(spreadplayerscommand$position.x) + 0.5, (double)spreadplayerscommand$position.getSpawnY(p_138731_, p_138733_), (double)Mth.floor(spreadplayerscommand$position.z) + 0.5);
            if (!event.isCanceled()) {
                entity.teleportToWithTicket(event.getTargetX(), event.getTargetY(), event.getTargetZ());
            }

            d2 = Double.MAX_VALUE;
            Position[] var15 = p_138732_;
            int var16 = p_138732_.length;

            for(int var17 = 0; var17 < var16; ++var17) {
                Position spreadplayerscommand$position1 = var15[var17];
    ...
    private static Position[] createInitialPositions(RandomSource p_214734_, int p_214735_, double p_214736_, double p_214737_, double p_214738_, double p_214739_) {
        Position[] aspreadplayerscommand$position = new Position[p_214735_];
