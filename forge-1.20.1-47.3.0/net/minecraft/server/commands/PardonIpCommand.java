...
public class PardonIpCommand {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> p_138109_) {
        p_138109_.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("pardon-ip").requires((p_138116_) -> {
            return p_138116_.hasPermission(3);
        })).then(Commands.argument("target", StringArgumentType.word()).suggests((p_138113_, p_138114_) -> {
            return SharedSuggestionProvider.suggest(((CommandSourceStack)p_138113_.getSource()).getServer().getPlayerList().getIpBans().getUserList(), p_138114_);
        }).executes((p_138111_) -> {
            return unban((CommandSourceStack)p_138111_.getSource(), StringArgumentType.getString(p_138111_, "target"));
        })));
    }
    ...
    private static int unban(CommandSourceStack p_138118_, String p_138119_) throws CommandSyntaxException {
        if (!InetAddresses.isInetAddress(p_138119_)) {
            ...
            IpBanList $$2 = p_138118_.getServer().getPlayerList().getIpBans();
            if (!$$2.isBanned(p_138119_)) {
