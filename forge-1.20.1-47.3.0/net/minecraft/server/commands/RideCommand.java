...
public class RideCommand {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> p_265201_) {
        p_265201_.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("ride").requires((p_265326_) -> {
            return p_265326_.hasPermission(2);
        })).then(((RequiredArgumentBuilder)Commands.argument("target", EntityArgument.entity()).then(Commands.literal("mount").then(Commands.argument("vehicle", EntityArgument.entity()).executes((p_265139_) -> {
            return mount((CommandSourceStack)p_265139_.getSource(), EntityArgument.getEntity(p_265139_, "target"), EntityArgument.getEntity(p_265139_, "vehicle"));
        })))).then(Commands.literal("dismount").executes((p_265418_) -> {
            return dismount((CommandSourceStack)p_265418_.getSource(), EntityArgument.getEntity(p_265418_, "target"));
        }))));
    }

    private static int mount(CommandSourceStack p_265285_, Entity p_265711_, Entity p_265339_) throws CommandSyntaxException {
        Entity $$3 = p_265711_.getVehicle();
        if ($$3 != null) {
            throw ERROR_ALREADY_RIDING.create(p_265711_.getDisplayName(), $$3.getDisplayName());
    ...
    private static int dismount(CommandSourceStack p_265724_, Entity p_265678_) throws CommandSyntaxException {
        Entity $$2 = p_265678_.getVehicle();
        if ($$2 == null) {
            throw ERROR_NOT_RIDING.create(p_265678_.getDisplayName());
        } else {
            p_265678_.stopRiding();
            p_265724_.sendSuccess(() -> {
                return Component.translatable("commands.ride.dismount.success", p_265678_.getDisplayName(), $$2.getDisplayName());
            }, true);
            return 1;
        }
    }
