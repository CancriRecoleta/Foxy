...
public class DebugPathCommand {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> p_180124_) {
        p_180124_.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("debugpath").requires((p_180128_) -> {
            return p_180128_.hasPermission(2);
        })).then(Commands.argument("to", BlockPosArgument.blockPos()).executes((p_180126_) -> {
            return fillBlocks((CommandSourceStack)p_180126_.getSource(), BlockPosArgument.getLoadedBlockPos(p_180126_, "to"));
        })));
    }

    private static int fillBlocks(CommandSourceStack p_180130_, BlockPos p_180131_) throws CommandSyntaxException {
        Entity $$2 = p_180130_.getEntity();
        if (!($$2 instanceof Mob $$3)) {
            throw ERROR_NOT_MOB.create();
        } else {
            PathNavigation $$4 = new GroundPathNavigation($$3, p_180130_.getLevel());
            Path $$5 = $$4.createPath((BlockPos)p_180131_, 0);
            DebugPackets.sendPathFindingPacket(p_180130_.getLevel(), $$3, $$5, $$4.getMaxDistanceToWaypoint());
            if ($$5 == null) {
