...
public class RconClient extends GenericThread {
            ...
            try {
                ...
                if (k == i - 4) {
                    ...
                    switch (i1) {
                        case 2:
                            if (this.authed) {
                                s1 = PktUtils.stringFromByteArray(this.buf, j, i);
    ...
    private void sendCmdResponse(int p_11595_, String p_11596_) throws IOException {
        ...
        do {
            int j = 4096 <= i ? 4096 : i;
            this.send(p_11595_, 0, new String(Arrays.copyOfRange(whole, start, j + start), StandardCharsets.UTF_8));
            i -= j;
            start += j;
        } while(0 != i);
