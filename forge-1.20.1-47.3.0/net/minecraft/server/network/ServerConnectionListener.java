...
public class ServerConnectionListener {
    ...
    public void startTcpServerListener(@Nullable InetAddress p_9712_, int p_9713_) throws IOException {
        ...
        DualStackUtils.checkIPv6(p_9712_);
        synchronized(this.channels) {
            ...
            this.channels.add(((ServerBootstrap)((ServerBootstrap)(new ServerBootstrap()).channel(oclass)).childHandler(new ChannelInitializer<Channel>() {
                protected void initChannel(Channel p_9729_) {
                    ...
                    ChannelPipeline channelpipeline = p_9729_.pipeline().addLast("timeout", new ReadTimeoutHandler(ServerConnectionListener.READ_TIMEOUT)).addLast("legacy_query", new LegacyQueryHandler(ServerConnectionListener.this));
                    ...
                    ((Connection)connection).setListener(new ServerHandshakePacketListenerImpl(ServerConnectionListener.this.server, (Connection)connection));
                }
            }).group((EventLoopGroup)lazyloadedvalue.get()).localAddress(p_9712_, p_9713_)).bind().syncUninterruptibly());
        }
    }
    ...
    static class LatencySimulator extends ChannelInboundHandlerAdapter {
        ...
        private void onTimeout(Timeout p_143599_) {
            DelayedMessage serverconnectionlistener$latencysimulator$delayedmessage = (DelayedMessage)this.queuedMessages.remove(0);
