...
public class ServerChunkCache extends ChunkSource {
    ...
    public ChunkAccess getChunk(int p_8360_, int p_8361_, ChunkStatus p_8362_, boolean p_8363_) {
        if (Thread.currentThread() != this.mainThread) {
            ...
            chunkaccess = (ChunkAccess)((Either)completablefuture.join()).map((p_8406_) -> {
                return p_8406_;
            }, (p_8423_) -> {
                if (p_8363_) {
            ...
            this.storeInCache(i, chunkaccess, p_8362_);
            return chunkaccess;
        }
    }
    ...
    public LevelChunk getChunkNow(int p_8357_, int p_8358_) {
        if (Thread.currentThread() != this.mainThread) {
            ...
            if (chunkholder == null) {
                return null;
            } else if (chunkholder.currentlyLoading != null) {
                return chunkholder.currentlyLoading;
            } else {
                Either<ChunkAccess, ChunkHolder.ChunkLoadingFailure> either = (Either)chunkholder.getFutureIfPresent(ChunkStatus.FULL).getNow((Either)null);
                if (either == null) {
                    return null;
                } else {
                    ChunkAccess chunkaccess1 = (ChunkAccess)either.left().orElse((ChunkAccess)null);
    ...
    public LightChunk getChunkForLighting(int p_8454_, int p_8455_) {
        ...
        if (chunkholder == null) {
            ...
            while(true) {
                ChunkStatus chunkstatus = (ChunkStatus)CHUNK_STATUSES.get(j);
    ...
    public boolean isPositionTicking(long p_143240_) {
        ...
        if (chunkholder == null) {
            ...
            Either<LevelChunk, ChunkHolder.ChunkLoadingFailure> either = (Either)chunkholder.getTickingChunkFuture().getNow((Either)null);
    ...
    private void tickChunks() {
        ...
        if (flag) {
            ...
            while(var13.hasNext()) {
                ChunkHolder chunkholder = (ChunkHolder)var13.next();
            ...
            while(true) {
                ...
                do {
                    ...
                    ChunkAndHolder serverchunkcache$chunkandholder = (ChunkAndHolder)var19.next();
                    levelchunk1 = serverchunkcache$chunkandholder.chunk;
                    chunkpos = levelchunk1.getPos();
                } while((!this.level.isNaturalSpawningAllowed(chunkpos) || !this.chunkMap.anyPlayerCloseEnoughForSpawning(chunkpos)) && !this.distanceManager.shouldForceTicks(chunkpos.toLong()));
    ...
    private void getFullChunk(long p_8371_, Consumer<LevelChunk> p_8372_) {
        ...
        if (chunkholder != null) {
            ((Either)chunkholder.getFullChunkFuture().getNow(ChunkHolder.UNLOADED_LEVEL_CHUNK)).left().ifPresent(p_8372_);
        }
    ...
    public <T> void addRegionTicket(TicketType<T> p_8388_, ChunkPos p_8389_, int p_8390_, T p_8391_) {
        this.addRegionTicket(p_8388_, p_8389_, p_8390_, p_8391_, false);
    }
    ...
    public <T> void removeRegionTicket(TicketType<T> p_8439_, ChunkPos p_8440_, int p_8441_, T p_8442_) {
        this.removeRegionTicket(p_8439_, p_8440_, p_8441_, p_8442_, false);
    }
