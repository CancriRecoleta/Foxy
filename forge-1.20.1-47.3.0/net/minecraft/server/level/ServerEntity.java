...
public class ServerEntity {
    ...
    public void sendChanges() {
        ...
        if ($$11 instanceof ItemFrame itemframe) {
            if (this.tickCount % 10 == 0) {
                ...
                MapItemSavedData mapitemsaveddata = MapItem.getSavedData((ItemStack)itemstack, this.level);
                if (mapitemsaveddata != null) {
                    Iterator var7 = this.level.players().iterator();

                    while(var7.hasNext()) {
                        ServerPlayer serverplayer = (ServerPlayer)var7.next();
        ...
        if (this.tickCount % this.updateInterval == 0 || this.entity.hasImpulse || this.entity.getEntityData().isDirty()) {
            ...
            if (this.entity.isPassenger()) {
                l = Mth.floor(this.entity.getYRot() * 256.0F / 360.0F);
                k1 = Mth.floor(this.entity.getXRot() * 256.0F / 360.0F);
                boolean flag3 = Math.abs(l - this.yRotp) >= 1 || Math.abs(k1 - this.xRotp) >= 1;
                if (flag3) {
                    this.broadcast.accept(new ClientboundMoveEntityPacket.Rot(this.entity.getId(), (byte)l, (byte)k1, this.entity.onGround()));
                    this.yRotp = l;
                    this.xRotp = k1;
                }
                ...
                boolean flag4 = this.positionCodec.delta(vec3).lengthSqr() >= 7.62939453125E-6;
                Packet<?> packet1 = null;
            ...
            l = Mth.floor(this.entity.getYHeadRot() * 256.0F / 360.0F);
            if (Math.abs(l - this.yHeadRotp) >= 1) {
                this.broadcast.accept(new ClientboundRotateHeadPacket(this.entity, (byte)l));
                this.yHeadRotp = l;
            }
    ...
    public void removePairing(ServerPlayer p_8535_) {
        ...
        p_8535_.connection.send(new ClientboundRemoveEntitiesPacket(new int[]{this.entity.getId()}));
        ForgeEventFactory.onStopEntityTracking(this.entity, p_8535_);
    }
    ...
    public void addPairing(ServerPlayer p_8542_) {
        ...
        ForgeEventFactory.onStartEntityTracking(this.entity, p_8542_);
    }
