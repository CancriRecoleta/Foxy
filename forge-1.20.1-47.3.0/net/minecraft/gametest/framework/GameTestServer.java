...
public class GameTestServer extends MinecraftServer {
    ...
    public static GameTestServer create(Thread p_206607_, LevelStorageSource.LevelStorageAccess p_206608_, PackRepository p_206609_, Collection<GameTestBatch> p_206610_, BlockPos p_206611_) {
        if (p_206610_.isEmpty()) {
            ...
            WorldLoader.InitConfig worldloader$initconfig = new WorldLoader.InitConfig(worldloader$packconfig, CommandSelection.DEDICATED, 4);

            try {
                LOGGER.debug("Starting resource loading");
                Stopwatch stopwatch = Stopwatch.createStarted();
                WorldStem worldstem = (WorldStem)Util.blockUntilDone((p_248045_) -> {
                    return WorldLoader.load(worldloader$initconfig, (p_258205_) -> {
                        Registry<LevelStem> registry = (new MappedRegistry(Registries.LEVEL_STEM, Lifecycle.stable())).freeze();
                        WorldDimensions.Complete worlddimensions$complete = ((WorldPreset)p_258205_.datapackWorldgen().registryOrThrow(Registries.WORLD_PRESET).getHolderOrThrow(WorldPresets.FLAT).value()).createWorldDimensions().bake(registry);
        ...
        if (this.testTracker.isDone()) {
            ...
            if (this.testTracker.hasFailedRequired()) {
                ...
                this.testTracker.getFailedRequired().forEach((p_206615_) -> {
                    LOGGER.info("   - {}", p_206615_.getTestName());
                });
            } else {
            ...
            if (this.testTracker.hasFailedOptional()) {
                ...
                this.testTracker.getFailedOptional().forEach((p_206613_) -> {
                    LOGGER.info("   - {}", p_206613_.getTestName());
                });
            }
    ...
    static {
        ...
        TEST_GAME_RULES = (GameRules)Util.make(new GameRules(), (p_177615_) -> {
            ((GameRules.BooleanValue)p_177615_.getRule(GameRules.RULE_DOMOBSPAWNING)).set(false, (MinecraftServer)null);
            ((GameRules.BooleanValue)p_177615_.getRule(GameRules.RULE_WEATHER_CYCLE)).set(false, (MinecraftServer)null);
        });
