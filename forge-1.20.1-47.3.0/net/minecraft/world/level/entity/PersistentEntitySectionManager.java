...
public class PersistentEntitySectionManager<T extends EntityAccess> implements AutoCloseable {
    ...
    public PersistentEntitySectionManager(Class<T> p_157503_, LevelCallback<T> p_157504_, EntityPersistentStorage<T> p_157505_) {
        ...
        this.chunkLoadStatuses.defaultReturnValue(net.minecraft.world.level.entity.PersistentEntitySectionManager.ChunkLoadStatus.FRESH);
    ...
    private boolean addEntity(T p_157539_, boolean p_157540_) {
        if (p_157539_ instanceof Entity entity) {
            if (MinecraftForge.EVENT_BUS.post(new EntityJoinLevelEvent(entity, entity.level(), p_157540_))) {
                return false;
            }
        }

        return this.addEntityWithoutEvent(p_157539_, p_157540_);
    }
    ...
    private boolean addEntityWithoutEvent(T p_157539_, boolean p_157540_) {
        if (!this.addEntityUuid(p_157539_)) {
            ...
            p_157539_.setLevelCallback(new Callback(p_157539_, i, entitysection));
            if (!p_157540_) {
    ...
    public void addLegacyChunkEntities(Stream<T> p_157553_) {
        p_157553_.forEach((p_157607_) -> {
            ...
            if (p_157607_ instanceof Entity entity) {
    ...
    public void addWorldGenChunkEntities(Stream<T> p_157560_) {
        p_157560_.forEach((p_157605_) -> {
            ...
            if (p_157605_ instanceof Entity entity) {
                entity.onAddedToWorld();
            }
    ...
    private void ensureChunkQueuedForLoad(long p_157556_) {
        ChunkLoadStatus persistententitysectionmanager$chunkloadstatus = (ChunkLoadStatus)this.chunkLoadStatuses.get(p_157556_);
        if (persistententitysectionmanager$chunkloadstatus == net.minecraft.world.level.entity.PersistentEntitySectionManager.ChunkLoadStatus.FRESH) {
    ...
    private boolean storeChunkSections(long p_157513_, Consumer<T> p_157514_) {
        ChunkLoadStatus persistententitysectionmanager$chunkloadstatus = (ChunkLoadStatus)this.chunkLoadStatuses.get(p_157513_);
        if (persistententitysectionmanager$chunkloadstatus == net.minecraft.world.level.entity.PersistentEntitySectionManager.ChunkLoadStatus.PENDING) {
            return false;
        } else {
            List<T> list = (List)this.sectionStorage.getExistingSectionsInChunk(p_157513_).flatMap((p_157542_) -> {
                return p_157542_.getEntities().filter(EntityAccess::shouldBeSaved);
            }).collect(Collectors.toList());
            if (list.isEmpty()) {
                if (persistententitysectionmanager$chunkloadstatus == net.minecraft.world.level.entity.PersistentEntitySectionManager.ChunkLoadStatus.LOADED) {
                    this.permanentStorage.storeEntities(new ChunkEntities(new ChunkPos(p_157513_), ImmutableList.of()));
                }

                return true;
            } else if (persistententitysectionmanager$chunkloadstatus == net.minecraft.world.level.entity.PersistentEntitySectionManager.ChunkLoadStatus.FRESH) {
    ...
    private void requestChunkLoad(long p_157563_) {
        this.chunkLoadStatuses.put(p_157563_, net.minecraft.world.level.entity.PersistentEntitySectionManager.ChunkLoadStatus.PENDING);
        ChunkPos chunkpos = new ChunkPos(p_157563_);
        CompletableFuture var10000 = this.permanentStorage.loadEntities(chunkpos);
        Queue var10001 = this.loadingInbox;
        Objects.requireNonNull(var10001);
        var10000.thenAccept(var10001::add).exceptionally((p_157532_) -> {
    ...
    private void unloadEntity(EntityAccess p_157586_) {
        p_157586_.setRemoved(RemovalReason.UNLOADED_TO_CHUNK);
    ...
    private void processPendingLoads() {
        ...
        while((chunkentities = (ChunkEntities)this.loadingInbox.poll()) != null) {
            ...
            this.chunkLoadStatuses.put(chunkentities.getPos().toLong(), net.minecraft.world.level.entity.PersistentEntitySectionManager.ChunkLoadStatus.LOADED);
        }
    ...
    private LongSet getAllChunksToSave() {
        ...
        while(var2.hasNext()) {
            ...
            if (entry.getValue() == net.minecraft.world.level.entity.PersistentEntitySectionManager.ChunkLoadStatus.LOADED) {
    ...
    public boolean areEntitiesLoaded(long p_157508_) {
        return this.chunkLoadStatuses.get(p_157508_) == net.minecraft.world.level.entity.PersistentEntitySectionManager.ChunkLoadStatus.LOADED;
    }
    ...
    public void dumpSections(Writer p_157549_) throws IOException {
        ...
        this.sectionStorage.getAllChunksWithExistingSections().forEach((p_157517_) -> {
            ChunkLoadStatus persistententitysectionmanager$chunkloadstatus = (ChunkLoadStatus)this.chunkLoadStatuses.get(p_157517_);
