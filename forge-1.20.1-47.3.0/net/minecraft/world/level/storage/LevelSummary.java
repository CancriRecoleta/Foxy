...
public class LevelSummary implements Comparable<LevelSummary> {
    ...
    public BackupStatus backupStatus() {
        ...
        if (!worldversion.isStable() && j < i) {
            return net.minecraft.world.level.storage.LevelSummary.BackupStatus.UPGRADE_TO_SNAPSHOT;
        } else {
            return j > i ? net.minecraft.world.level.storage.LevelSummary.BackupStatus.DOWNGRADE : net.minecraft.world.level.storage.LevelSummary.BackupStatus.NONE;
        }
    }
    ...
    private Component createInfo() {
        if (this.isLocked()) {
            ...
            MutableComponent mutablecomponent = this.isHardcore() ? Component.empty().append((Component)Component.translatable("gameMode.hardcore").withStyle((p_265611_) -> {
                return p_265611_.withColor(-65536);
            })) : Component.translatable("gameMode." + this.getGameMode().getName());
            if (this.hasCheats()) {
                mutablecomponent.append(", ").append((Component)Component.translatable("selectWorld.cheats"));
            }
            ...
            if (this.markVersionInList()) {
                mutablecomponent2.append((Component)mutablecomponent1.withStyle(this.askToOpenWorld() ? ChatFormatting.RED : ChatFormatting.ITALIC));
            } else {
