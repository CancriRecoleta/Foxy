...
public class LootTable {
    ...
    private ObjectArrayList<ItemStack> getRandomItems(LootContext p_230923_) {
        ...
        ServerLevel var10002 = p_230923_.getLevel();
        Objects.requireNonNull(objectarraylist);
        this.getRandomItemsRaw(p_230923_, createStackSplitter(var10002, objectarraylist::add));
        objectarraylist = ForgeHooks.modifyLoot(this.getLootTableId(), objectarraylist, p_230923_);
    ...
    public void validate(ValidationContext p_79137_) {
        ...
        for(j = 0; j < this.pools.size(); ++j) {
            ((LootPool)this.pools.get(j)).validate(p_79137_.forChild(".pools[" + j + "]"));
        }
    ...
    public void fill(Container p_287662_, LootParams p_287743_, long p_287585_) {
        ...
        while(var9.hasNext()) {
            ...
            if (itemstack.isEmpty()) {
                p_287662_.setItem((Integer)list.remove(list.size() - 1), ItemStack.EMPTY);
            } else {
                p_287662_.setItem((Integer)list.remove(list.size() - 1), itemstack);
            }
        }
    ...
    private void shuffleAndSplitItems(ObjectArrayList<ItemStack> p_230925_, int p_230926_, RandomSource p_230927_) {
        ...
        while(p_230926_ - p_230925_.size() - list.size() > 0 && !list.isEmpty()) {
            itemstack = (ItemStack)list.remove(Mth.nextInt(p_230927_, 0, list.size() - 1));
            int i = Mth.nextInt(p_230927_, 1, itemstack.getCount() / 2);
            ItemStack itemstack1 = itemstack.split(i);
            if (itemstack.getCount() > 1 && p_230927_.nextBoolean()) {
                list.add(itemstack);
            } else {
                p_230925_.add(itemstack);
            }
    ...
    public void setLootTableId(ResourceLocation id) {
        if (this.lootTableId != null) {
            ...
            this.lootTableId = (ResourceLocation)Objects.requireNonNull(id);
        }
    }
    ...
    public @org.jetbrains.annotations.Nullable LootPool getPool(String name) {
        return (LootPool)this.pools.stream().filter((e) -> {
            return name.equals(e.getName());
        }).findFirst().orElse((Object)null);
    }

    public @org.jetbrains.annotations.Nullable LootPool removePool(String name) {
        this.checkFrozen();
        ...
        } while(!name.equals(pool.getName()));
    ...
    public void addPool(LootPool pool) {
        this.checkFrozen();
        if (this.pools.stream().anyMatch((e) -> {
            return e == pool || e.getName() != null && e.getName().equals(pool.getName());
        })) {
    ...
    public static class Serializer implements JsonDeserializer<LootTable>, JsonSerializer<LootTable> {
        ...
        public LootTable deserialize(JsonElement p_79173_, Type p_79174_, JsonDeserializationContext p_79175_) throws JsonParseException {
            ...
            LootPool[] alootpool = (LootPool[])GsonHelper.getAsObject(jsonobject, "pools", new LootPool[0], p_79175_, LootPool[].class);
            ...
            LootItemFunction[] alootitemfunction = (LootItemFunction[])GsonHelper.getAsObject(jsonobject, "functions", new LootItemFunction[0], p_79175_, LootItemFunction[].class);
        ...
        public JsonElement serialize(LootTable p_79177_, Type p_79178_, JsonSerializationContext p_79179_) {
            ...
            if (p_79177_.paramSet != LootTable.DEFAULT_PARAM_SET) {
                ...
                if (resourcelocation != null) {
                    ...
                    LootTable.LOGGER.warn("Failed to find id for param set {}", p_79177_.paramSet);
                }
            }
