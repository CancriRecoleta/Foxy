...
public class NoiseThresholdProvider extends NoiseBasedStateProvider {
    ...
    public BlockState getState(RandomSource p_225916_, BlockPos p_225917_) {
        double $$2 = this.getNoiseValue(p_225917_, (double)this.scale);
        if ($$2 < (double)this.threshold) {
            return (BlockState)Util.getRandom(this.lowStates, p_225916_);
        } else {
            return p_225916_.nextFloat() < this.highChance ? (BlockState)Util.getRandom(this.highStates, p_225916_) : this.defaultState;
        }
    }
}
