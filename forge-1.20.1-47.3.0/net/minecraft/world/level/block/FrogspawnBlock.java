...
public class FrogspawnBlock extends Block {
    ...
    private static boolean mayPlaceOn(BlockGetter p_221188_, BlockPos p_221189_) {
        FluidState $$2 = p_221188_.getFluidState(p_221189_);
        FluidState $$3 = p_221188_.getFluidState(p_221189_.above());
    ...
    private void spawnTadpoles(ServerLevel p_221221_, BlockPos p_221222_, RandomSource p_221223_) {
        int $$3 = p_221223_.nextInt(2, 6);

        for(int $$4 = 1; $$4 <= $$3; ++$$4) {
            Tadpole $$5 = (Tadpole)EntityType.TADPOLE.create(p_221221_);
            if ($$5 != null) {
                double $$6 = (double)p_221222_.getX() + this.getRandomTadpolePositionOffset(p_221223_);
                double $$7 = (double)p_221222_.getZ() + this.getRandomTadpolePositionOffset(p_221223_);
                int $$8 = p_221223_.nextInt(1, 361);
                $$5.moveTo($$6, (double)p_221222_.getY() - 0.5, $$7, (float)$$8, 0.0F);
                $$5.setPersistenceRequired();
                p_221221_.addFreshEntity($$5);
            }
        }
    ...
    private double getRandomTadpolePositionOffset(RandomSource p_221225_) {
        ...
        return Mth.clamp(p_221225_.nextDouble(), $$1, 1.0 - $$1);
    }
