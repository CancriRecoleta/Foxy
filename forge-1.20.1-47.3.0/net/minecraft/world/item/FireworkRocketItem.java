...
public class FireworkRocketItem extends Item {
    ...
    public InteractionResult useOn(UseOnContext p_41216_) {
        ...
        if (!level.isClientSide) {
            ...
            FireworkRocketEntity fireworkrocketentity = new FireworkRocketEntity(level, p_41216_.getPlayer(), vec3.x + (double)direction.getStepX() * 0.15, vec3.y + (double)direction.getStepY() * 0.15, vec3.z + (double)direction.getStepZ() * 0.15, itemstack);
    ...
    public void appendHoverText(ItemStack p_41211_, @Nullable Level p_41212_, List<Component> p_41213_, TooltipFlag p_41214_) {
        ...
        if (compoundtag != null) {
            if (compoundtag.contains("Flight", 99)) {
                p_41213_.add(Component.translatable("item.minecraft.firework_rocket.flight").append(CommonComponents.SPACE).append(String.valueOf(compoundtag.getByte("Flight"))).withStyle(ChatFormatting.GRAY));
            }
    ...
    public static enum Shape implements IExtensibleEnum {
        ...
        private static final IntFunction<Shape> BY_ID = ByIdMap.continuous(Shape::getId, values(), OutOfBoundsStrategy.ZERO);
        ...
        public void save(CompoundTag tag) {
            ...
            tag.putString("forge:shape_type", this.name());
        }

        /** @deprecated */
        @Deprecated
        public static Shape byId(int p_41238_) {
            return (Shape)BY_ID.apply(p_41238_);
        }

        public static Shape getShape(CompoundTag tag) {
            String name = tag.contains("forge:shape_type", 8) ? tag.getString("forge:shape_type") : null;
            if (name == null) {
                return byId(tag.getByte("Type"));
            } else {
                Shape[] var2 = values();
