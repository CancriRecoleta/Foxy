...
public class PotionBrewing {
    ...
    private static final Predicate<ItemStack> ALLOWED_CONTAINER = (p_43528_) -> {
        ...
        } while(!ingredient.test(p_43528_));
    ...
    protected static boolean isContainerIngredient(ItemStack p_43518_) {
        ...
        for(int j = CONTAINER_MIXES.size(); i < j; ++i) {
            if (((Mix)CONTAINER_MIXES.get(i)).ingredient.test(p_43518_)) {
                return true;
            }
        }
    ...
    protected static boolean isPotionIngredient(ItemStack p_43523_) {
        ...
        for(int j = POTION_MIXES.size(); i < j; ++i) {
            if (((Mix)POTION_MIXES.get(i)).ingredient.test(p_43523_)) {
                return true;
            }
        }
    ...
    public static boolean isBrewablePotion(Potion p_43512_) {
        ...
        for(int j = POTION_MIXES.size(); i < j; ++i) {
            if (((Mix)POTION_MIXES.get(i)).to.get() == p_43512_) {
                return true;
            }
        }
    ...
    protected static boolean hasContainerMix(ItemStack p_43520_, ItemStack p_43521_) {
        ...
        for(int j = CONTAINER_MIXES.size(); i < j; ++i) {
            Mix<Item> mix = (Mix)CONTAINER_MIXES.get(i);
    ...
    protected static boolean hasPotionMix(ItemStack p_43525_, ItemStack p_43526_) {
        ...
        for(int j = POTION_MIXES.size(); i < j; ++i) {
            Mix<Potion> mix = (Mix)POTION_MIXES.get(i);
    ...
    public static ItemStack mix(ItemStack p_43530_, ItemStack p_43531_) {
        if (!p_43531_.isEmpty()) {
            ...
            int k;
            Mix mix1;
            for(k = CONTAINER_MIXES.size(); i < k; ++i) {
                mix1 = (Mix)CONTAINER_MIXES.get(i);
                if (mix1.from.get() == item && mix1.ingredient.test(p_43530_)) {
                    return PotionUtils.setPotion(new ItemStack((ItemLike)mix1.to.get()), potion);
                }
            }

            i = 0;

            for(k = POTION_MIXES.size(); i < k; ++i) {
                mix1 = (Mix)POTION_MIXES.get(i);
    ...
    private static void addContainerRecipe(Item p_43503_, Item p_43504_, Item p_43505_) {
        if (!(p_43503_ instanceof PotionItem)) {
            ...
            CONTAINER_MIXES.add(new Mix(ForgeRegistries.ITEMS, p_43503_, Ingredient.of(p_43504_), p_43505_));
        }
    }
    ...
    private static void addMix(Potion p_43514_, Item p_43515_, Potion p_43516_) {
        POTION_MIXES.add(new Mix(ForgeRegistries.POTIONS, p_43514_, Ingredient.of(p_43515_), p_43516_));
    }
