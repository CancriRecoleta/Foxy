...
public class RecipeManager extends SimpleJsonResourceReloadListener {
        ...
        while(true) {
            ...
            try {
                if (((JsonElement)entry.getValue()).isJsonObject() && !CraftingHelper.processConditions(((JsonElement)entry.getValue()).getAsJsonObject(), "conditions", this.context)) {
                    ...
                    Recipe<?> recipe = fromJson(resourcelocation, GsonHelper.convertToJsonObject((JsonElement)entry.getValue(), "top element"), this.context);
                    if (recipe == null) {
    ...
    public <C extends Container, T extends Recipe<C>> Optional<Pair<ResourceLocation, T>> getRecipeFor(RecipeType<T> p_220249_, C p_220250_, Level p_220251_, @Nullable ResourceLocation p_220252_) {
        ...
        return map.entrySet().stream().filter((p_220245_) -> {
            return ((Recipe)p_220245_.getValue()).matches(p_220250_, p_220251_);
        }).findFirst().map((p_220256_) -> {
            return Pair.of((ResourceLocation)p_220256_.getKey(), (Recipe)p_220256_.getValue());
        });
    }
    ...
    public <C extends Container, T extends Recipe<C>> List<T> getRecipesFor(RecipeType<T> p_44057_, C p_44058_, Level p_44059_) {
        return (List)this.byType(p_44057_).values().stream().filter((p_220241_) -> {
    ...
    private <C extends Container, T extends Recipe<C>> Map<ResourceLocation, T> byType(RecipeType<T> p_44055_) {
        return (Map)this.recipes.getOrDefault(p_44055_, Collections.emptyMap());
    }
    ...
    public <C extends Container, T extends Recipe<C>> NonNullList<ItemStack> getRemainingItemsFor(RecipeType<T> p_44070_, C p_44071_, Level p_44072_) {
        ...
        if (optional.isPresent()) {
            return ((Recipe)optional.get()).getRemainingItems(p_44071_);
        } else {
    ...
    public Optional<? extends Recipe<?>> byKey(ResourceLocation p_44044_) {
        return Optional.ofNullable((Recipe)this.byName.get(p_44044_));
    }

    public Collection<Recipe<?>> getRecipes() {
        return (Collection)this.recipes.values().stream().flatMap((p_220270_) -> {
    ...
    public static Recipe<?> fromJson(ResourceLocation p_44046_, JsonObject p_44047_) {
        return fromJson(p_44046_, p_44047_, IContext.EMPTY);
    }
    ...
    public void replaceRecipes(Iterable<Recipe<?>> p_44025_) {
        ...
        p_44025_.forEach((p_220262_) -> {
            Map<ResourceLocation, Recipe<?>> map1 = (Map)map.computeIfAbsent(p_220262_.getType(), (p_220272_) -> {
                return Maps.newHashMap();
            });
            ResourceLocation resourcelocation = p_220262_.getId();
            Recipe<?> recipe = (Recipe)map1.put(resourcelocation, p_220262_);
