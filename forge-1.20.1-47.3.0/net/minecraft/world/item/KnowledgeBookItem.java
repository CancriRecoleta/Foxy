//
// Source code recreated from a .class file by IntelliJ IDEA
// (powered by FernFlower decompiler)
//

package net.minecraft.world.item;

import com.google.common.collect.Lists;
import com.mojang.logging.LogUtils;
import java.util.List;
import java.util.Optional;
import net.minecraft.nbt.CompoundTag;
import net.minecraft.nbt.ListTag;
import net.minecraft.resources.ResourceLocation;
import net.minecraft.stats.Stats;
import net.minecraft.world.InteractionHand;
import net.minecraft.world.InteractionResultHolder;
import net.minecraft.world.entity.player.Player;
import net.minecraft.world.item.crafting.Recipe;
import net.minecraft.world.item.crafting.RecipeManager;
import net.minecraft.world.level.Level;
import org.slf4j.Logger;

public class KnowledgeBookItem extends Item {
    private static final String RECIPE_TAG = "Recipes";
    private static final Logger LOGGER = LogUtils.getLogger();

    public KnowledgeBookItem(Item.Properties p_42822_) {
        super(p_42822_);
    }

    public InteractionResultHolder<ItemStack> use(Level p_42824_, Player p_42825_, InteractionHand p_42826_) {
        ItemStack $$3 = p_42825_.getItemInHand(p_42826_);
        CompoundTag $$4 = $$3.getTag();
        if (!p_42825_.getAbilities().instabuild) {
            p_42825_.setItemInHand(p_42826_, ItemStack.EMPTY);
        }

        if ($$4 != null && $$4.contains("Recipes", 9)) {
            if (!p_42824_.isClientSide) {
                ListTag $$5 = $$4.getList("Recipes", 8);
                List<Recipe<?>> $$6 = Lists.newArrayList();
                RecipeManager $$7 = p_42824_.getServer().getRecipeManager();

                for(int $$8 = 0; $$8 < $$5.size(); ++$$8) {
                    String $$9 = $$5.getString($$8);
                    Optional<? extends Recipe<?>> $$10 = $$7.byKey(new ResourceLocation($$9));
                    if (!$$10.isPresent()) {
                        LOGGER.error("Invalid recipe: {}", $$9);
                        return InteractionResultHolder.fail($$3);
                    }

                    $$6.add((Recipe)$$10.get());
                }

                p_42825_.awardRecipes($$6);
                p_42825_.awardStat(Stats.ITEM_USED.get(this));
            }

            return InteractionResultHolder.sidedSuccess($$3, p_42824_.isClientSide());
        } else {
            LOGGER.error("Tag not valid: {}", $$4);
            return InteractionResultHolder.fail($$3);
        }
    }
}
