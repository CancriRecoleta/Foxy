...
public class BoneMealItem extends Item {
    ...
    public static boolean growCrop(ItemStack p_40628_, Level p_40629_, BlockPos p_40630_) {
        return p_40629_ instanceof ServerLevel ? applyBonemeal(p_40628_, p_40629_, p_40630_, FakePlayerFactory.getMinecraft((ServerLevel)p_40629_)) : false;
    }
                ...
                for(int i = 0; i < 128; ++i) {
                    ...
                    if (holder.is(BiomeTags.PRODUCES_CORALS_FROM_BONEMEAL)) {
                        if (i == 0 && p_40635_ != null && p_40635_.getAxis().isHorizontal()) {
                            blockstate = (BlockState)BuiltInRegistries.BLOCK.getTag(BlockTags.WALL_CORALS).flatMap((p_204098_) -> {
                                return p_204098_.getRandomElement(p_40633_.random);
                            }).map((p_204100_) -> {
                                return ((Block)p_204100_.value()).defaultBlockState();
                            }).orElse(blockstate);
                            if (blockstate.hasProperty(BaseCoralWallFanBlock.FACING)) {
                                blockstate = (BlockState)blockstate.setValue(BaseCoralWallFanBlock.FACING, p_40635_);
                            }
                        } else if (randomsource.nextInt(4) == 0) {
                            blockstate = (BlockState)BuiltInRegistries.BLOCK.getTag(BlockTags.UNDERWATER_BONEMEALS).flatMap((p_204091_) -> {
    ...
    public static void addGrowthParticles(LevelAccessor p_40639_, BlockPos p_40640_, int p_40641_) {
        ...
        if (!blockstate.isAir()) {
            ...
            if (blockstate.is(Blocks.WATER)) {
                ...
                d1 = blockstate.getShape(p_40639_, p_40640_).max(Axis.Y);
            }

            p_40639_.addParticle(ParticleTypes.HAPPY_VILLAGER, (double)p_40640_.getX() + 0.5, (double)p_40640_.getY() + 0.5, (double)p_40640_.getZ() + 0.5, 0.0, 0.0, 0.0);
