...
public class MenuType<T extends AbstractContainerMenu> implements FeatureElement, IForgeMenuType<T> {
    ...
    private static <T extends AbstractContainerMenu> MenuType<T> register(String p_39989_, MenuSupplier<T> p_39990_) {
        return (MenuType)Registry.register(BuiltInRegistries.MENU, (String)p_39989_, new MenuType(p_39990_, FeatureFlags.VANILLA_SET));
    }

    private static <T extends AbstractContainerMenu> MenuType<T> register(String p_267295_, MenuSupplier<T> p_266945_, FeatureFlag... p_267055_) {
        return (MenuType)Registry.register(BuiltInRegistries.MENU, (String)p_267295_, new MenuType(p_266945_, FeatureFlags.REGISTRY.subset(p_267055_)));
    }

    public MenuType(MenuSupplier<T> p_267054_, FeatureFlagSet p_266909_) {
    ...
    public T create(int windowId, Inventory playerInv, FriendlyByteBuf extraData) {
        return this.constructor instanceof IContainerFactory ? ((IContainerFactory)this.constructor).create(windowId, playerInv, extraData) : this.create(windowId, playerInv);
    }
