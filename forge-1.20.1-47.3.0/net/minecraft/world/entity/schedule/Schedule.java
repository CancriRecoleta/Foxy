...
public class Schedule {
    ...
    protected static ScheduleBuilder register(String p_38030_) {
        Schedule $$1 = (Schedule)Registry.register(BuiltInRegistries.SCHEDULE, (String)p_38030_, new Schedule());
    ...
    protected Timeline getTimelineFor(Activity p_38032_) {
        return (Timeline)this.timelines.get(p_38032_);
    }

    protected List<Timeline> getAllTimelinesExceptFor(Activity p_38034_) {
        return (List)this.timelines.entrySet().stream().filter((p_38028_) -> {
            return p_38028_.getKey() != p_38034_;
        }).map(Map.Entry::getValue).collect(Collectors.toList());
    }

    public Activity getActivityAt(int p_38020_) {
        return (Activity)this.timelines.entrySet().stream().max(Comparator.comparingDouble((p_38023_) -> {
            return (double)((Timeline)p_38023_.getValue()).getValueAt(p_38020_);
