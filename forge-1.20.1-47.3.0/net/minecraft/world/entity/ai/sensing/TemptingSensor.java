...
public class TemptingSensor extends Sensor<PathfinderMob> {
    ...
    protected void doTick(ServerLevel p_148331_, PathfinderMob p_148332_) {
        ...
        Stream var10000 = p_148331_.players().stream().filter(EntitySelector.NO_SPECTATORS).filter((p_148342_) -> {
            return TEMPT_TARGETING.test(p_148332_, p_148342_);
        }).filter((p_148335_) -> {
            return p_148332_.closerThan(p_148335_, 10.0);
        }).filter(this::playerHoldingTemptation).filter((p_264964_) -> {
            return !p_148332_.hasPassenger(p_264964_);
        });
        Objects.requireNonNull(p_148332_);
        List<Player> $$3 = (List)var10000.sorted(Comparator.comparingDouble(p_148332_::distanceToSqr)).collect(Collectors.toList());
        if (!$$3.isEmpty()) {
            Player $$4 = (Player)$$3.get(0);
            $$2.setMemory(MemoryModuleType.TEMPTING_PLAYER, (Object)$$4);
        } else {
