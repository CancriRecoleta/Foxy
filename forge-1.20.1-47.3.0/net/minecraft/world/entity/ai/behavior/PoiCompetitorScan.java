...
public class PoiCompetitorScan {
        ...
        return BehaviorBuilder.create((p_258576_) -> {
            return p_258576_.group(p_258576_.present(MemoryModuleType.JOB_SITE), p_258576_.present(MemoryModuleType.NEAREST_LIVING_ENTITIES)).apply(p_258576_, (p_258590_, p_258591_) -> {
                return (p_258580_, p_258581_, p_258582_) -> {
                    GlobalPos $$6 = (GlobalPos)p_258576_.get(p_258590_);
                    p_258580_.getPoiManager().getType($$6.pos()).ifPresent((p_258588_) -> {
                        ((List)p_258576_.get(p_258591_)).stream().filter((p_258593_) -> {
                            ...
                            return competesForSameJobsite($$6, p_258588_, p_258596_);
    ...
    private static boolean competesForSameJobsite(GlobalPos p_217330_, Holder<PoiType> p_217331_, Villager p_217332_) {
        Optional<GlobalPos> $$3 = p_217332_.getBrain().getMemory(MemoryModuleType.JOB_SITE);
        return $$3.isPresent() && p_217330_.equals($$3.get()) && hasMatchingProfession(p_217331_, p_217332_.getVillagerData().getProfession());
    }
