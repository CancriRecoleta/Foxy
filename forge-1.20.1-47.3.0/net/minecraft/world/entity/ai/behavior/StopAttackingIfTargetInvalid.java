...
public class StopAttackingIfTargetInvalid {
        ...
        return BehaviorBuilder.create((p_258801_) -> {
            return p_258801_.group(p_258801_.present(MemoryModuleType.ATTACK_TARGET), p_258801_.registered(MemoryModuleType.CANT_REACH_WALK_TARGET_SINCE)).apply(p_258801_, (p_258787_, p_258788_) -> {
                return (p_258795_, p_258796_, p_258797_) -> {
                    LivingEntity $$9 = (LivingEntity)p_258801_.get(p_258787_);
                    if (p_258796_.canAttack($$9) && (!p_260319_ || !isTiredOfTryingToReachTarget(p_258796_, p_258801_.tryGet(p_258788_))) && $$9.isAlive() && $$9.level() == p_258796_.level() && !p_260357_.test($$9)) {
                        return true;
                    } else {
                        p_259568_.accept(p_258796_, $$9);
                        p_258787_.erase();
                        return true;
                    }
                };
    ...
    private static boolean isTiredOfTryingToReachTarget(LivingEntity p_259416_, Optional<Long> p_259377_) {
        return p_259377_.isPresent() && p_259416_.level().getGameTime() - (Long)p_259377_.get() > 200L;
    }
}
