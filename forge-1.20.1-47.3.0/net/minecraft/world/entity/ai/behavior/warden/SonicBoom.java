...
public class SonicBoom extends Behavior<Warden> {
    ...
    protected void tick(ServerLevel p_217724_, Warden p_217725_, long p_217726_) {
        ...
        if (!p_217725_.getBrain().hasMemoryValue(MemoryModuleType.SONIC_BOOM_SOUND_DELAY) && !p_217725_.getBrain().hasMemoryValue(MemoryModuleType.SONIC_BOOM_SOUND_COOLDOWN)) {
            ...
            Optional var10000 = p_217725_.getBrain().getMemory(MemoryModuleType.ATTACK_TARGET);
            Objects.requireNonNull(p_217725_);
            var10000.filter(p_217725_::canTargetEntity).filter((p_217707_) -> {
                return p_217725_.closerThan(p_217707_, 15.0, 20.0);
            }).ifPresent((p_217704_) -> {
                Vec3 $$3 = p_217725_.position().add(0.0, 1.600000023841858, 0.0);
                Vec3 $$4 = p_217704_.getEyePosition().subtract($$3);
                Vec3 $$5 = $$4.normalize();

                for(int $$6 = 1; $$6 < Mth.floor($$4.length()) + 7; ++$$6) {
                    Vec3 $$7 = $$3.add($$5.scale((double)$$6));
                    p_217724_.sendParticles(ParticleTypes.SONIC_BOOM, $$7.x, $$7.y, $$7.z, 1, 0.0, 0.0, 0.0, 0.0);
                }
