...
public class Roar extends Behavior<Warden> {
    ...
    protected void start(ServerLevel p_217580_, Warden p_217581_, long p_217582_) {
        Brain<Warden> $$3 = p_217581_.getBrain();
        $$3.setMemoryWithExpiry(MemoryModuleType.ROAR_SOUND_DELAY, Unit.INSTANCE, 25L);
        $$3.eraseMemory(MemoryModuleType.WALK_TARGET);
        LivingEntity $$4 = (LivingEntity)p_217581_.getBrain().getMemory(MemoryModuleType.ROAR_TARGET).get();
        BehaviorUtils.lookAtEntity(p_217581_, $$4);
        p_217581_.setPose(Pose.ROARING);
        p_217581_.increaseAngerAt($$4, 20, false);
    }
    ...
    protected void stop(ServerLevel p_217604_, Warden p_217605_, long p_217606_) {
        ...
        Optional var10000 = p_217605_.getBrain().getMemory(MemoryModuleType.ROAR_TARGET);
        Objects.requireNonNull(p_217605_);
        var10000.ifPresent(p_217605_::setAttackTarget);
