...
public class AnimalPanic extends Behavior<PathfinderMob> {
    ...
    protected void start(ServerLevel p_147399_, PathfinderMob p_147400_, long p_147401_) {
        p_147400_.getBrain().setMemory(MemoryModuleType.IS_PANICKING, (Object)true);
    ...
    protected void tick(ServerLevel p_147403_, PathfinderMob p_147404_, long p_147405_) {
        if (p_147404_.getNavigation().isDone()) {
            Vec3 $$3 = this.getPanicPos(p_147404_, p_147403_);
            if ($$3 != null) {
                p_147404_.getBrain().setMemory(MemoryModuleType.WALK_TARGET, (Object)(new WalkTarget($$3, this.speedMultiplier, 0)));
            }
        }
    ...
    private Vec3 getPanicPos(PathfinderMob p_196639_, ServerLevel p_196640_) {
        if (p_196639_.isOnFire()) {
            Optional<Vec3> $$2 = this.lookForWater(p_196640_, p_196639_).map(Vec3::atBottomCenterOf);
            if ($$2.isPresent()) {
                return (Vec3)$$2.get();
            }
        }
    ...
    private Optional<BlockPos> lookForWater(BlockGetter p_196642_, Entity p_196643_) {
        BlockPos $$2 = p_196643_.blockPosition();
        if (!p_196642_.getBlockState($$2).getCollisionShape(p_196642_, $$2).isEmpty()) {
