...
public class GiveGiftToHero extends Behavior<Villager> {
    ...
    private static final Map<VillagerProfession, ResourceLocation> GIFTS = (Map)Util.make(Maps.newHashMap(), (p_23020_) -> {
    ...
    protected void start(ServerLevel p_23006_, Villager p_23007_, long p_23008_) {
        ...
        Player $$3 = (Player)this.getNearestTargetableHero(p_23007_).get();
        p_23007_.getBrain().setMemory(MemoryModuleType.INTERACTION_TARGET, (Object)$$3);
        BehaviorUtils.lookAtEntity(p_23007_, $$3);
    }
    ...
    protected void tick(ServerLevel p_23036_, Villager p_23037_, long p_23038_) {
        Player $$3 = (Player)this.getNearestTargetableHero(p_23037_).get();
        BehaviorUtils.lookAtEntity(p_23037_, $$3);
        if (this.isWithinThrowingDistance(p_23037_, $$3)) {
            if (p_23038_ - this.timeSinceStart > 20L) {
                this.throwGift(p_23037_, $$3);
                this.giftGivenDuringThisRun = true;
            }
        } else {
            BehaviorUtils.setWalkAndLookTargetMemories(p_23037_, (Entity)$$3, 0.5F, 5);
        }
    ...
    private void throwGift(Villager p_23012_, LivingEntity p_23013_) {
        List<ItemStack> $$2 = this.getItemToThrow(p_23012_);
        Iterator var4 = $$2.iterator();

        while(var4.hasNext()) {
            ItemStack $$3 = (ItemStack)var4.next();
            BehaviorUtils.throwItem(p_23012_, $$3, p_23013_.position());
        }
    ...
    private List<ItemStack> getItemToThrow(Villager p_23010_) {
        if (p_23010_.isBaby()) {
            ...
            VillagerProfession $$1 = p_23010_.getVillagerData().getProfession();
            if (GIFTS.containsKey($$1)) {
                LootTable $$2 = p_23010_.level().getServer().getLootData().getLootTable((ResourceLocation)GIFTS.get($$1));
                LootParams $$3 = (new LootParams.Builder((ServerLevel)p_23010_.level())).withParameter(LootContextParams.ORIGIN, p_23010_.position()).withParameter(LootContextParams.THIS_ENTITY, p_23010_).create(LootContextParamSets.GIFT);
                return $$2.getRandomItems($$3);
            } else {
    ...
    private boolean isWithinThrowingDistance(Villager p_23015_, Player p_23016_) {
        BlockPos $$2 = p_23016_.blockPosition();
        BlockPos $$3 = p_23015_.blockPosition();
