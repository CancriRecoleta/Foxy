...
public class PoiManager extends SectionStorage<PoiSection> {
    ...
    public void add(BlockPos p_217920_, Holder<PoiType> p_217921_) {
        ((PoiSection)this.getOrCreate(SectionPos.asLong(p_217920_))).add(p_217920_, p_217921_);
    }
    ...
    public long getCountInRange(Predicate<Holder<PoiType>> p_27122_, BlockPos p_27123_, int p_27124_, Occupancy p_27125_) {
    ...
    public Stream<PoiRecord> getInSquare(Predicate<Holder<PoiType>> p_27167_, BlockPos p_27168_, int p_27169_, Occupancy p_27170_) {
        int $$4 = Math.floorDiv(p_27169_, 16) + 1;
        return ChunkPos.rangeClosed(new ChunkPos(p_27168_), $$4).flatMap((p_217938_) -> {
            return this.getInChunk(p_27167_, p_217938_, p_27170_);
        }).filter((p_217971_) -> {
            BlockPos $$3 = p_217971_.getPos();
            return Math.abs($$3.getX() - p_27168_.getX()) <= p_27169_ && Math.abs($$3.getZ() - p_27168_.getZ()) <= p_27169_;
        });
    }

    public Stream<PoiRecord> getInRange(Predicate<Holder<PoiType>> p_27182_, BlockPos p_27183_, int p_27184_, Occupancy p_27185_) {
        int $$4 = p_27184_ * p_27184_;
        return this.getInSquare(p_27182_, p_27183_, p_27184_, p_27185_).filter((p_217906_) -> {
            return p_217906_.getPos().distSqr(p_27183_) <= (double)$$4;
        });
    }

    @VisibleForDebug
    public Stream<PoiRecord> getInChunk(Predicate<Holder<PoiType>> p_27118_, ChunkPos p_27119_, Occupancy p_27120_) {
        return IntStream.range(this.levelHeightAccessor.getMinSection(), this.levelHeightAccessor.getMaxSection()).boxed().map((p_217886_) -> {
            ...
            return ((PoiSection)p_217942_.get()).getRecords(p_27118_, p_27120_);
        });
    }

    public Stream<BlockPos> findAll(Predicate<Holder<PoiType>> p_27139_, Predicate<BlockPos> p_27140_, BlockPos p_27141_, int p_27142_, Occupancy p_27143_) {
        return this.getInRange(p_27139_, p_27141_, p_27142_, p_27143_).map(PoiRecord::getPos).filter(p_27140_);
    }

    public Stream<Pair<Holder<PoiType>, BlockPos>> findAllWithType(Predicate<Holder<PoiType>> p_217984_, Predicate<BlockPos> p_217985_, BlockPos p_217986_, int p_217987_, Occupancy p_217988_) {
    ...
    public Stream<Pair<Holder<PoiType>, BlockPos>> findAllClosestFirstWithType(Predicate<Holder<PoiType>> p_217995_, Predicate<BlockPos> p_217996_, BlockPos p_217997_, int p_217998_, Occupancy p_217999_) {
        return this.findAllWithType(p_217995_, p_217996_, p_217997_, p_217998_, p_217999_).sorted(Comparator.comparingDouble((p_217915_) -> {
            return ((BlockPos)p_217915_.getSecond()).distSqr(p_217997_);
        }));
    }

    public Optional<BlockPos> find(Predicate<Holder<PoiType>> p_27187_, Predicate<BlockPos> p_27188_, BlockPos p_27189_, int p_27190_, Occupancy p_27191_) {
        return this.findAll(p_27187_, p_27188_, p_27189_, p_27190_, p_27191_).findFirst();
    }

    public Optional<BlockPos> findClosest(Predicate<Holder<PoiType>> p_27193_, BlockPos p_27194_, int p_27195_, Occupancy p_27196_) {
    ...
    public Optional<Pair<Holder<PoiType>, BlockPos>> findClosestWithType(Predicate<Holder<PoiType>> p_218003_, BlockPos p_218004_, int p_218005_, Occupancy p_218006_) {
    ...
    public Optional<BlockPos> findClosest(Predicate<Holder<PoiType>> p_148659_, Predicate<BlockPos> p_148660_, BlockPos p_148661_, int p_148662_, Occupancy p_148663_) {
    ...
    public Optional<BlockPos> take(Predicate<Holder<PoiType>> p_217947_, BiPredicate<Holder<PoiType>, BlockPos> p_217948_, BlockPos p_217949_, int p_217950_) {
        return this.getInRange(p_217947_, p_217949_, p_217950_, net.minecraft.world.entity.ai.village.poi.PoiManager.Occupancy.HAS_SPACE).filter((p_217934_) -> {
    ...
    public Optional<BlockPos> getRandom(Predicate<Holder<PoiType>> p_217952_, Predicate<BlockPos> p_217953_, Occupancy p_217954_, BlockPos p_217955_, int p_217956_, RandomSource p_217957_) {
        List<PoiRecord> $$6 = Util.toShuffledList(this.getInRange(p_217952_, p_217955_, p_217956_, p_217954_), p_217957_);
        return $$6.stream().filter((p_217945_) -> {
            return p_217953_.test(p_217945_.getPos());
        }).findFirst().map(PoiRecord::getPos);
    }

    public boolean release(BlockPos p_27155_) {
        return (Boolean)this.getOrLoad(SectionPos.asLong(p_27155_)).map((p_217993_) -> {
            return p_217993_.release(p_27155_);
        }).orElseThrow(() -> {
            return (IllegalStateException)Util.pauseInIde(new IllegalStateException("POI never registered at " + p_27155_));
        });
    }

    public boolean exists(BlockPos p_27092_, Predicate<Holder<PoiType>> p_27093_) {
        return (Boolean)this.getOrLoad(SectionPos.asLong(p_27092_)).map((p_217925_) -> {
    ...
    public int getFreeTickets(BlockPos p_148654_) {
        return (Integer)this.getOrLoad(SectionPos.asLong(p_148654_)).map((p_217912_) -> {
    ...
    boolean isVillageCenter(long p_27198_) {
        Optional<PoiSection> $$1 = this.get(p_27198_);
        return $$1 == null ? false : (Boolean)$$1.map((p_217883_) -> {
            return p_217883_.getRecords((p_217927_) -> {
                return p_217927_.is(PoiTypeTags.VILLAGE);
            }, net.minecraft.world.entity.ai.village.poi.PoiManager.Occupancy.IS_OCCUPIED).findAny().isPresent();
        }).orElse(false);
    }
    ...
    public void checkConsistencyWithBlocks(SectionPos p_281731_, LevelChunkSection p_281893_) {
        Util.ifElse(this.getOrLoad(p_281731_.asLong()), (p_217898_) -> {
            ...
            if (mayHavePoi(p_281893_)) {
                PoiSection $$2 = (PoiSection)this.getOrCreate(p_281731_.asLong());
                Objects.requireNonNull($$2);
                this.updateFromSection(p_281893_, p_281731_, $$2::add);
            }
    ...
    private void updateFromSection(LevelChunkSection p_27070_, SectionPos p_27071_, BiConsumer<BlockPos, Holder<PoiType>> p_27072_) {
        p_27071_.blocksInside().forEach((p_217902_) -> {
            BlockState $$3 = p_27070_.getBlockState(SectionPos.sectionRelative(p_217902_.getX()), SectionPos.sectionRelative(p_217902_.getY()), SectionPos.sectionRelative(p_217902_.getZ()));
            PoiTypes.forState($$3).ifPresent((p_217931_) -> {
    ...
    public void ensureLoadedAndValid(LevelReader p_27057_, BlockPos p_27058_, int p_27059_) {
        SectionPos.aroundChunk(new ChunkPos(p_27058_), Math.floorDiv(p_27059_, 16), this.levelHeightAccessor.getMinSection(), this.levelHeightAccessor.getMaxSection()).map((p_217979_) -> {
            ...
            return !(Boolean)((Optional)p_217963_.getSecond()).map(PoiSection::isValid).orElse(false);
        }).map((p_217891_) -> {
            return ((SectionPos)p_217891_.getFirst()).chunk();
        }).filter((p_217961_) -> {
    ...
    public static enum Occupancy {
        ...
        private Occupancy(Predicate p_27220_) {
            this.test = p_27220_;
        }
