...
public class PiglinAi {
    ...
    private static void initFightActivity(Piglin p_34904_, Brain<Piglin> p_34905_) {
        p_34905_.addActivityAndRemoveMemoryWhenStopped(Activity.FIGHT, 10, ImmutableList.of(StopAttackingIfTargetInvalid.create((p_34981_) -> {
    ...
    private static void initCelebrateActivity(Brain<Piglin> p_34921_) {
        p_34921_.addActivityAndRemoveMemoryWhenStopped(Activity.CELEBRATE, 10, ImmutableList.of(avoidRepellent(), SetEntityLookTarget.create(PiglinAi::isPlayerHoldingLovedItem, 14.0F), StartAttacking.create(AbstractPiglin::isAdult, PiglinAi::findNearestValidAttackTarget), BehaviorBuilder.triggerIf((p_34804_) -> {
    ...
    private static void initRideHoglinActivity(Brain<Piglin> p_34974_) {
        p_34974_.addActivityAndRemoveMemoryWhenStopped(Activity.RIDE, 10, ImmutableList.of(Mount.create(0.8F), SetEntityLookTarget.create(PiglinAi::isPlayerHoldingLovedItem, 8.0F), BehaviorBuilder.sequence(BehaviorBuilder.triggerIf(Entity::isPassenger), TriggerGate.triggerOneShuffled(ImmutableList.builder().addAll(createLookBehaviors()).add(Pair.of(BehaviorBuilder.triggerIf((p_258950_) -> {
            return true;
    ...
    protected static void updateActivity(Piglin p_34899_) {
        ...
        Activity activity1 = (Activity)brain.getActiveNonCoreActivity().orElse((Activity)null);
        if (activity != activity1) {
            Optional var10000 = getSoundForCurrentActivity(p_34899_);
            Objects.requireNonNull(p_34899_);
            var10000.ifPresent(p_34899_::playSoundEvent);
        }
    ...
    protected static void stopHoldingOffHandItem(Piglin p_34868_, boolean p_34869_) {
        ...
        boolean flag;
        if (p_34868_.isAdult()) {
            flag = itemstack.isPiglinCurrency();
            if (p_34869_ && flag) {
            ...
            flag = !p_34868_.equipItemIfPossible(itemstack).isEmpty();
            if (!flag) {
    ...
    private static void throwItems(Piglin p_34861_, List<ItemStack> p_34862_) {
        ...
        if (optional.isPresent()) {
            throwItemsTowardPlayer(p_34861_, (Player)optional.get(), p_34862_);
        } else {
    ...
    private static void throwItemsTowardPos(Piglin p_34864_, List<ItemStack> p_34865_, Vec3 p_34866_) {
        if (!p_34865_.isEmpty()) {
            ...
            Iterator var3 = p_34865_.iterator();
    ...
    protected static void maybeRetaliate(AbstractPiglin p_34827_, LivingEntity p_34828_) {
        if (!p_34827_.getBrain().isActive(Activity.AVOID) && Sensor.isEntityAttackableIgnoringLineOfSight(p_34827_, p_34828_) && !BehaviorUtils.isOtherTargetMuchFurtherAwayThanCurrentAttackTarget(p_34827_, p_34828_, 4.0)) {
    ...
    private static boolean isNearAvoidTarget(Piglin p_35003_) {
        ...
        return !brain.hasMemoryValue(MemoryModuleType.AVOID_TARGET) ? false : ((LivingEntity)brain.getMemory(MemoryModuleType.AVOID_TARGET).get()).closerThan(p_35003_, 12.0);
    }

    protected static List<AbstractPiglin> getVisibleAdultPiglins(Piglin p_35005_) {
        return (List)p_35005_.getBrain().getMemory(MemoryModuleType.NEAREST_VISIBLE_ADULT_PIGLINS).orElse(ImmutableList.of());
    }

    private static List<AbstractPiglin> getAdultPiglins(AbstractPiglin p_34961_) {
        return (List)p_34961_.getBrain().getMemory(MemoryModuleType.NEARBY_ADULT_PIGLINS).orElse(ImmutableList.of());
    }

    public static boolean isWearingGold(LivingEntity p_34809_) {
        Iterator var1 = p_34809_.getArmorSlots().iterator();
        ...
        } while(!itemstack.makesPiglinsNeutral(p_34809_));
    ...
    private static void setAngerTargetToNearestTargetablePlayerIfFound(AbstractPiglin p_34945_, LivingEntity p_34946_) {
        ...
        if (optional.isPresent()) {
            setAngerTarget(p_34945_, (LivingEntity)optional.get());
        } else {
    ...
    private static boolean hoglinsOutnumberPiglins(Piglin p_35013_) {
        int i = (Integer)p_35013_.getBrain().getMemory(MemoryModuleType.VISIBLE_ADULT_PIGLIN_COUNT).orElse(0) + 1;
        int j = (Integer)p_35013_.getBrain().getMemory(MemoryModuleType.VISIBLE_ADULT_HOGLIN_COUNT).orElse(0);
        return j > i;
    }
