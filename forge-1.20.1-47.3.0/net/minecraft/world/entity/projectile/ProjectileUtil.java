...
public final class ProjectileUtil {
    ...
    private static HitResult getHitResult(Vec3 p_278237_, Entity p_278320_, Predicate<Entity> p_278257_, Vec3 p_278342_, Level p_278321_) {
        ...
        HitResult hitresult = p_278321_.clip(new ClipContext(p_278237_, vec3, Block.COLLIDER, Fluid.NONE, p_278320_));
        if (((HitResult)hitresult).getType() != Type.MISS) {
            vec3 = ((HitResult)hitresult).getLocation();
        }

        HitResult hitresult1 = getEntityHitResult(p_278321_, p_278320_, p_278237_, vec3, p_278320_.getBoundingBox().expandTowards(p_278342_).inflate(1.0), p_278257_);
        if (hitresult1 != null) {
    ...
    public static EntityHitResult getEntityHitResult(Entity p_37288_, Vec3 p_37289_, Vec3 p_37290_, AABB p_37291_, Predicate<Entity> p_37292_, double p_37293_) {
        ...
        Entity entity = null;
        Vec3 vec3 = null;
        Iterator var12 = level.getEntities(p_37288_, p_37291_, p_37292_).iterator();
        ...
        while(true) {
            while(var12.hasNext()) {
                ...
                if (aabb.contains(p_37289_)) {
                    if (d0 >= 0.0) {
                        entity = entity1;
                        vec3 = (Vec3)optional.orElse(p_37289_);
                        d0 = 0.0;
                    }
    ...
    public static EntityHitResult getEntityHitResult(Level p_150176_, Entity p_150177_, Vec3 p_150178_, Vec3 p_150179_, AABB p_150180_, Predicate<Entity> p_150181_, float p_150182_) {
        ...
        Entity entity = null;
        Iterator var10 = p_150176_.getEntities(p_150177_, p_150180_, p_150181_).iterator();

        while(var10.hasNext()) {
            Entity entity1 = (Entity)var10.next();
            AABB aabb = entity1.getBoundingBox().inflate((double)p_150182_);
            Optional<Vec3> optional = aabb.clip(p_150178_, p_150179_);
            if (optional.isPresent()) {
                double d1 = p_150178_.distanceToSqr((Vec3)optional.get());
                if (d1 < d0) {
                    entity = entity1;
                    d0 = d1;
                }
    ...
    public static void rotateTowardsMovement(Entity p_37285_, float p_37286_) {
        ...
        if (vec3.lengthSqr() != 0.0) {
            ...
            p_37285_.setYRot((float)(Mth.atan2(vec3.z, vec3.x) * 57.2957763671875) + 90.0F);
            p_37285_.setXRot((float)(Mth.atan2(d0, vec3.y) * 57.2957763671875) - 90.0F);
