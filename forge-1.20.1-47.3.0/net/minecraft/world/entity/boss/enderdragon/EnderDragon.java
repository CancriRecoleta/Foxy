...
public class EnderDragon extends Mob implements Enemy {
    ...
    public void aiStep() {
        ...
        if (this.isDeadOrDying()) {
            ...
            f9 = (this.random.nextFloat() - 0.5F) * 4.0F;
            float f11 = (this.random.nextFloat() - 0.5F) * 8.0F;
            this.level().addParticle(ParticleTypes.EXPLOSION, this.getX() + (double)f8, this.getY() + 2.0 + (double)f9, this.getZ() + (double)f11, 0.0, 0.0, 0.0);
        } else {
            this.checkCrystals();
            Vec3 vec34 = this.getDeltaMovement();
            f9 = 0.2F / ((float)vec34.horizontalDistance() * 10.0F + 1.0F);
            ...
            if (this.isNoAi()) {
                ...
                if (this.level().isClientSide) {
                    if (this.lerpSteps > 0) {
                        ...
                        d7 = this.getY() + (this.lerpY - this.getY()) / (double)this.lerpSteps;
                        d8 = this.getZ() + (this.lerpZ - this.getZ()) / (double)this.lerpSteps;
                        d9 = Mth.wrapDegrees(this.lerpYRot - (double)this.getYRot());
                        this.setYRot(this.getYRot() + (float)d9 / (float)this.lerpSteps);
                    ...
                    if (vec3 != null) {
                        ...
                        Vec3 vec32 = (new Vec3((double)Mth.sin(this.getYRot() * 0.017453292F), this.getDeltaMovement().y, (double)(-Mth.cos(this.getYRot() * 0.017453292F)))).normalize();
                        f18 = Math.max(((float)vec32.dot(vec31) + 0.5F) / 1.5F, 0.0F);
                        if (Math.abs(d7) > 9.999999747378752E-6 || Math.abs(d9) > 9.999999747378752E-6) {
                            f20 = Mth.clamp(Mth.wrapDegrees(180.0F - (float)Mth.atan2(d7, d9) * 57.295776F - this.getYRot()), -50.0F, 50.0F);
                            this.yRotA *= 0.8F;
                            this.yRotA += f20 * dragonphaseinstance.getTurnSpeed();
                            this.setYRot(this.getYRot() + this.yRotA * 0.1F);
                        }

                        f20 = (float)(2.0 / (d3 + 1.0));
                        f21 = 0.06F;
                        this.moveRelative(0.06F * (f18 * f20 + (1.0F - f20)), new Vec3(0.0, 0.0, -1.0));
                        if (this.inWall) {
                        ...
                        this.setDeltaMovement(this.getDeltaMovement().multiply(d5, 0.9100000262260437, d5));
                    }
                }
                ...
                float f12 = (float)(this.getLatencyPos(5, 1.0F)[1] - this.getLatencyPos(10, 1.0F)[1]) * 10.0F * 0.017453292F;
                float f13 = Mth.cos(f12);
                float f = Mth.sin(f12);
                float f14 = this.getYRot() * 0.017453292F;
                ...
                float f2 = Mth.sin(this.getYRot() * 0.017453292F - this.yRotA * 0.01F);
                float f16 = Mth.cos(this.getYRot() * 0.017453292F - this.yRotA * 0.01F);
                ...
                for(k = 0; k < 3; ++k) {
                    ...
                    float f17 = this.getYRot() * 0.017453292F + this.rotWrap(adouble1[0] - adouble[0]) * 0.017453292F;
                    f18 = Mth.sin(f17);
                    f20 = Mth.cos(f17);
                    f21 = 1.5F;
                    float f22 = (float)(k + 1) * 2.0F;
                    this.tickPart(enderdragonpart, (double)(-(f1 * 1.5F + f18 * f22) * f13), adouble1[1] - adouble[1] - (double)((f22 + 1.5F) * f) + 1.5, (double)((f15 * 1.5F + f20 * f22) * f13));
                }
    ...
    private void checkCrystals() {
        ...
        if (this.random.nextInt(10) == 0) {
            ...
            while(var5.hasNext()) {
                EndCrystal endcrystal1 = (EndCrystal)var5.next();
    ...
    private void knockBack(List<Entity> p_31132_) {
        ...
        Iterator var6 = p_31132_.iterator();
        ...
        while(var6.hasNext()) {
            ...
            if (entity instanceof LivingEntity) {
                ...
                entity.push(d2 / d4 * 4.0, 0.20000000298023224, d3 / d4 * 4.0);
    ...
    private void hurt(List<Entity> p_31142_) {
        Iterator var2 = p_31142_.iterator();
    ...
    protected void tickDeath() {
        ...
        if (this.level() instanceof ServerLevel) {
            if (this.dragonDeathTime > 150 && this.dragonDeathTime % 5 == 0 && flag) {
                award = ForgeEventFactory.getExperienceDrop(this, this.unlimitedLastHurtByPlayer, Mth.floor((float)i * 0.08F));
        ...
        if (this.dragonDeathTime == 200 && this.level() instanceof ServerLevel) {
            if (flag) {
                award = ForgeEventFactory.getExperienceDrop(this, this.unlimitedLastHurtByPlayer, Mth.floor((float)i * 0.2F));
                ExperienceOrb.award((ServerLevel)this.level(), this.position(), award);
            }
    ...
    public int findClosestNode() {
        if (this.nodes[0] == null) {
            for(int i = 0; i < 24; ++i) {
                ...
                int $$2;
                if (i < 12) {
                    l = Mth.floor(60.0F * Mth.cos(2.0F * (-3.1415927F + 0.2617994F * (float)i)));
                    i1 = Mth.floor(60.0F * Mth.sin(2.0F * (-3.1415927F + 0.2617994F * (float)i)));
                } else if (i < 20) {
                    $$2 = i - 12;
                    l = Mth.floor(40.0F * Mth.cos(2.0F * (-3.1415927F + 0.3926991F * (float)$$2)));
                    i1 = Mth.floor(40.0F * Mth.sin(2.0F * (-3.1415927F + 0.3926991F * (float)$$2)));
                    j += 10;
                } else {
                    $$2 = i - 20;
                    l = Mth.floor(20.0F * Mth.cos(2.0F * (-3.1415927F + 0.7853982F * (float)$$2)));
                    i1 = Mth.floor(20.0F * Mth.sin(2.0F * (-3.1415927F + 0.7853982F * (float)$$2)));
                }

                $$2 = Math.max(this.level().getSeaLevel() + 10, this.level().getHeightmapPos(Types.MOTION_BLOCKING_NO_LEAVES, new BlockPos(l, 0, i1)).getY() + j);
                this.nodes[i] = new Node(l, $$2, i1);
            }
    ...
    public float getHeadPartYOffset(int p_31109_, double[] p_31110_, double[] p_31111_) {
        ...
        if (enderdragonphase != EnderDragonPhase.LANDING && enderdragonphase != EnderDragonPhase.TAKEOFF) {
            ...
            BlockPos blockpos = this.level().getHeightmapPos(Types.MOTION_BLOCKING_NO_LEAVES, EndPodiumFeature.getLocation(this.fightOrigin));
    ...
    public Vec3 getHeadLookVector(float p_31175_) {
        ...
        if (enderdragonphase != EnderDragonPhase.LANDING && enderdragonphase != EnderDragonPhase.TAKEOFF) {
            ...
            BlockPos blockpos = this.level().getHeightmapPos(Types.MOTION_BLOCKING_NO_LEAVES, EndPodiumFeature.getLocation(this.fightOrigin));
            f5 = Math.max((float)Math.sqrt(blockpos.distToCenterSqr(this.position())) / 4.0F, 1.0F);
            float f1 = 6.0F / f5;
