...
public interface Bucketable {
    ...
    static void saveDefaultDataToBucketTag(Mob p_148823_, ItemStack p_148824_) {
        CompoundTag $$2 = p_148824_.getOrCreateTag();
        if (p_148823_.hasCustomName()) {
            p_148824_.setHoverName(p_148823_.getCustomName());
        }

        if (p_148823_.isNoAi()) {
            $$2.putBoolean("NoAI", p_148823_.isNoAi());
        }

        if (p_148823_.isSilent()) {
            $$2.putBoolean("Silent", p_148823_.isSilent());
        }

        if (p_148823_.isNoGravity()) {
            $$2.putBoolean("NoGravity", p_148823_.isNoGravity());
        }

        if (p_148823_.hasGlowingTag()) {
            $$2.putBoolean("Glowing", p_148823_.hasGlowingTag());
        }

        if (p_148823_.isInvulnerable()) {
            $$2.putBoolean("Invulnerable", p_148823_.isInvulnerable());
        }

        $$2.putFloat("Health", p_148823_.getHealth());
    }
    ...
    static <T extends LivingEntity & Bucketable> Optional<InteractionResult> bucketMobPickup(Player p_148829_, InteractionHand p_148830_, T p_148831_) {
        ...
        if ($$3.getItem() == Items.WATER_BUCKET && p_148831_.isAlive()) {
            ...
            ItemStack $$4 = ((Bucketable)p_148831_).getBucketItemStack();
            ((Bucketable)p_148831_).saveToBucketTag($$4);
