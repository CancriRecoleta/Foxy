...
public class FrogAi {
    ...
    protected static void initMemories(Frog p_218580_, RandomSource p_218581_) {
        p_218580_.getBrain().setMemory(MemoryModuleType.LONG_JUMP_COOLDOWN_TICKS, (Object)TIME_BETWEEN_LONG_JUMPS.sample(p_218581_));
    }
    ...
    private static void initSwimActivity(Brain<Frog> p_218595_) {
        ...
        })), Pair.of(3, TryFindLand.create(8, 1.5F)), Pair.of(5, new GateBehavior(ImmutableMap.of(MemoryModuleType.WALK_TARGET, MemoryStatus.VALUE_ABSENT), ImmutableSet.of(), OrderPolicy.ORDERED, RunningPolicy.TRY_ALL, ImmutableList.of(Pair.of(RandomStroll.swim(0.75F), 1), Pair.of(RandomStroll.stroll(1.0F, true), 1), Pair.of(SetWalkTargetFromLookTarget.create(1.0F, 3), 1), Pair.of(BehaviorBuilder.triggerIf(Entity::isInWaterOrBubble), 5))))), ImmutableSet.of(Pair.of(MemoryModuleType.LONG_JUMP_MID_JUMP, MemoryStatus.VALUE_ABSENT), Pair.of(MemoryModuleType.IS_IN_WATER, MemoryStatus.VALUE_PRESENT)));
    }
    ...
    private static <E extends Mob> boolean isAcceptableLandingSpot(E p_249699_, BlockPos p_250057_) {
        Level $$2 = p_249699_.level();
        BlockPos $$3 = p_250057_.below();
        if ($$2.getFluidState(p_250057_).isEmpty() && $$2.getFluidState($$3).isEmpty() && $$2.getFluidState(p_250057_.above()).isEmpty()) {
            BlockState $$4 = $$2.getBlockState(p_250057_);
            BlockState $$5 = $$2.getBlockState($$3);
            if (!$$4.is(BlockTags.FROG_PREFER_JUMP_TO) && !$$5.is(BlockTags.FROG_PREFER_JUMP_TO)) {
                BlockPathTypes $$6 = WalkNodeEvaluator.getBlockPathTypeStatic($$2, p_250057_.mutable());
                BlockPathTypes $$7 = WalkNodeEvaluator.getBlockPathTypeStatic($$2, $$3.mutable());
                return $$6 != BlockPathTypes.TRAPDOOR && (!$$4.isAir() || $$7 != BlockPathTypes.TRAPDOOR) ? LongJumpToRandomPos.defaultAcceptableLandingSpot(p_249699_, p_250057_) : true;
            } else {
                return true;
            }
        } else {
