...
public class KeyboardHandler {
    ...
    private void debugComponent(ChatFormatting p_167825_, Component p_167826_) {
        this.minecraft.gui.getChat().addMessage(Component.empty().append((Component)Component.translatable("debug.prefix").withStyle(p_167825_, ChatFormatting.BOLD)).append(CommonComponents.SPACE).append(p_167826_));
    }
            ...
            switch (p_90933_) {
                ...
                case 83:
                    ...
                    Component component = Component.literal(path.relativize(path1).toString()).withStyle(ChatFormatting.UNDERLINE).withStyle((p_276097_) -> {
                        return p_276097_.withClickEvent(new ClickEvent(Action.OPEN_FILE, path1.toFile().toString()));
                    });
        ...
        if (p_90894_ == this.minecraft.getWindow().getWindow()) {
            ...
            if (this.minecraft.getNarrator().isActive()) {
                ...
                if (p_90897_ != 0 && p_90895_ == 66 && Screen.hasControlDown() && flag) {
                    flag3 = this.minecraft.options.narrator().get() == NarratorStatus.OFF;
            ...
            if (screen != null) {
                ...
                Screen.wrapScreenError(() -> {
                    if (p_90897_ != 1 && p_90897_ != 2) {
                        if (p_90897_ == 0) {
                            aboolean[0] = ForgeHooksClient.onScreenKeyReleasedPre(screen, p_90895_, p_90896_, p_90898_);
                            if (!aboolean[0]) {
                                aboolean[0] = screen.keyReleased(p_90895_, p_90896_, p_90898_);
                            }

                            if (!aboolean[0]) {
                                aboolean[0] = ForgeHooksClient.onScreenKeyReleasedPost(screen, p_90895_, p_90896_, p_90898_);
                            }
                        }
                    } else {
                        screen.afterKeyboardAction();
                        aboolean[0] = ForgeHooksClient.onScreenKeyPressedPre(screen, p_90895_, p_90896_, p_90898_);
                        if (!aboolean[0]) {
                            aboolean[0] = screen.keyPressed(p_90895_, p_90896_, p_90898_);
                        }

                        if (!aboolean[0]) {
                            aboolean[0] = ForgeHooksClient.onScreenKeyPressedPost(screen, p_90895_, p_90896_, p_90898_);
                        }
                    }
        ...
        if (p_90890_ == this.minecraft.getWindow().getWindow()) {
            ...
            if (guieventlistener != null && this.minecraft.getOverlay() == null) {
                if (Character.charCount(p_90891_) == 1) {
                    Screen.wrapScreenError(() -> {
                        if (!ForgeHooksClient.onScreenCharTypedPre(guieventlistener, (char)p_90891_, p_90892_)) {
                            if (!guieventlistener.charTyped((char)p_90891_, p_90892_)) {
                                ForgeHooksClient.onScreenCharTypedPost(guieventlistener, (char)p_90891_, p_90892_);
                            }
                        }
                    }, "charTyped event handler", guieventlistener.getClass().getCanonicalName());
                } else {
                    char[] var6 = Character.toChars(p_90891_);
                    int var7 = var6.length;

                    for(int var8 = 0; var8 < var7; ++var8) {
                        char c0 = var6[var8];
                        Screen.wrapScreenError(() -> {
                            if (!ForgeHooksClient.onScreenCharTypedPre(guieventlistener, c0, p_90892_)) {
                                if (!guieventlistener.charTyped(c0, p_90892_)) {
                                    ForgeHooksClient.onScreenCharTypedPost(guieventlistener, c0, p_90892_);
                                }
                            }
                        }, "charTyped event handler", guieventlistener.getClass().getCanonicalName());
                    }
