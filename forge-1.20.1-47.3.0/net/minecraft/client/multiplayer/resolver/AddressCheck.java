...
public interface AddressCheck {
    ...
    static AddressCheck createFromService() {
        final ImmutableList<Predicate<String>> immutablelist = (ImmutableList)Streams.stream(ServiceLoader.load(BlockListSupplier.class, FMLLoader.class.getClassLoader())).map(BlockListSupplier::createBlockList).filter(Objects::nonNull).collect(ImmutableList.toImmutableList());
