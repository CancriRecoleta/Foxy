...
public class CamelModel<T extends Camel> extends HierarchicalModel<T> {
    ...
    public CamelModel(ModelPart p_251834_) {
        this.root = p_251834_;
        ModelPart $$1 = p_251834_.getChild("body");
    ...
    private void applyHeadRotation(T p_250436_, float p_249176_, float p_251814_, float p_248796_) {
        ...
        if (p_250436_.getJumpCooldown() > 0) {
            float $$4 = p_248796_ - (float)p_250436_.tickCount;
            float $$5 = 45.0F * ((float)p_250436_.getJumpCooldown() - $$4) / 55.0F;
            p_251814_ = Mth.clamp(p_251814_ + $$5, -25.0F, 70.0F);
        }

        this.head.yRot = p_249176_ * 0.017453292F;
        this.head.xRot = p_251814_ * 0.017453292F;
    }

    private void toggleInvisibleParts(T p_251765_) {
        boolean $$1 = p_251765_.isSaddled();
        boolean $$2 = p_251765_.isVehicle();
