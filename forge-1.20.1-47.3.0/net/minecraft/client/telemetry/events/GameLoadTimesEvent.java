...
public class GameLoadTimesEvent {
    ...
    public synchronized void beginStep(TelemetryProperty<Measurement> p_286394_) {
        this.beginStep(p_286394_, (p_286494_) -> {
            return Stopwatch.createStarted(this.timeSource);
        });
    }

    public synchronized void beginStep(TelemetryProperty<Measurement> p_286396_, Stopwatch p_286822_) {
        this.beginStep(p_286396_, (p_286421_) -> {
            return p_286822_;
        });
    }

    private synchronized void beginStep(TelemetryProperty<Measurement> p_286311_, Function<TelemetryProperty<Measurement>, Stopwatch> p_286454_) {
        this.measurements.computeIfAbsent(p_286311_, p_286454_);
    }

    public synchronized void endStep(TelemetryProperty<Measurement> p_286634_) {
        Stopwatch $$1 = (Stopwatch)this.measurements.get(p_286634_);
        if ($$1 == null) {
            ...
            synchronized(this) {
                this.measurements.forEach((p_286804_, p_286275_) -> {
                    if (!p_286275_.isRunning()) {
                        ...
                        p_286285_.put(p_286804_, new Measurement((int)$$3));
                    } else {
                ...
                this.bootstrapTime.ifPresent((p_286872_) -> {
                    p_286285_.put(TelemetryProperty.LOAD_TIME_BOOTSTRAP_MS, new Measurement((int)p_286872_));
                });
    ...
    public static record Measurement(int millis) {
        ...
        static {
            CODEC = Codec.INT.xmap(Measurement::new, (p_286736_) -> {
