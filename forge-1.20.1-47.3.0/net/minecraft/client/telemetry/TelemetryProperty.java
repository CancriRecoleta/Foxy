...
import net.minecraft.client.telemetry.events.GameLoadTimesEvent.Measurement;
...
public record TelemetryProperty<T>(String id, String exportKey, Codec<T> codec, Exporter<T> exporter) {
    ...
    public static <T> TelemetryProperty<T> create(String p_262052_, String p_261851_, Codec<T> p_261617_, Exporter<T> p_261478_) {
    ...
    public static TelemetryProperty<GameLoadTimesEvent.Measurement> gameLoadMeasurement(String p_286636_, String p_286769_) {
        return create(p_286636_, p_286769_, Measurement.CODEC, (p_286179_, p_286180_, p_286181_) -> {
    ...
    public static TelemetryProperty<LongList> longSamples(String p_262055_, String p_261726_) {
        return create(p_262055_, p_261726_, Codec.LONG.listOf().xmap(LongArrayList::new, Function.identity()), (p_261674_, p_262049_, p_262118_) -> {
            p_261674_.addProperty(p_262049_, (String)p_262118_.longStream().mapToObj(String::valueOf).collect(Collectors.joining(";")));
        });
    }
    ...
    static {
        ...
        SERVER_TYPE = create("server_type", "serverType", net.minecraft.client.telemetry.TelemetryProperty.ServerType.CODEC, (p_261518_, p_262138_, p_262085_) -> {
            p_261518_.addProperty(p_262138_, p_262085_.getSerializedName());
        });
        OPT_IN = bool("opt_in", "isOptional");
        EVENT_TIMESTAMP_UTC = create("event_timestamp_utc", "eventTimestampUtc", ExtraCodecs.INSTANT_ISO8601, (p_261517_, p_261626_, p_261868_) -> {
            p_261517_.addProperty(p_261626_, TIMESTAMP_FORMATTER.format(p_261868_));
        });
        GAME_MODE = create("game_mode", "playerGameMode", net.minecraft.client.telemetry.TelemetryProperty.GameMode.CODEC, (p_261849_, p_262092_, p_261574_) -> {
