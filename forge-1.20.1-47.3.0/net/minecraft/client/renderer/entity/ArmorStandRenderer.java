...
public class ArmorStandRenderer extends LivingEntityRenderer<ArmorStand, ArmorStandArmorModel> {
    ...
    protected void setupRotations(ArmorStand p_113800_, PoseStack p_113801_, float p_113802_, float p_113803_, float p_113804_) {
        ...
        float $$5 = (float)(p_113800_.level().getGameTime() - p_113800_.lastHit) + p_113804_;
        if ($$5 < 5.0F) {
            p_113801_.mulPose(Axis.YP.rotationDegrees(Mth.sin($$5 / 1.5F * 3.1415927F) * 3.0F));
        }

    }

    protected boolean shouldShowName(ArmorStand p_113815_) {
        double $$1 = this.entityRenderDispatcher.distanceToSqr(p_113815_);
        float $$2 = p_113815_.isCrouching() ? 32.0F : 64.0F;
        return $$1 >= (double)($$2 * $$2) ? false : p_113815_.isCustomNameVisible();
    }
    ...
    protected RenderType getRenderType(ArmorStand p_113806_, boolean p_113807_, boolean p_113808_, boolean p_113809_) {
        if (!p_113806_.isMarker()) {
            ...
            ResourceLocation $$4 = this.getTextureLocation(p_113806_);
            if (p_113808_) {
                return RenderType.entityTranslucent($$4, false);
            } else {
                return p_113807_ ? RenderType.entityCutoutNoCull($$4, false) : null;
            }
        }
    }
}
