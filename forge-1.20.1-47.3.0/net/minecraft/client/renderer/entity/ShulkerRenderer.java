...
public class ShulkerRenderer extends MobRenderer<Shulker, ShulkerModel<Shulker>> {
    ...
    public Vec3 getRenderOffset(Shulker p_115904_, float p_115905_) {
        return (Vec3)p_115904_.getRenderPosition(p_115905_).orElse(super.getRenderOffset(p_115904_, p_115905_));
    }

    public boolean shouldRender(Shulker p_115913_, Frustum p_115914_, double p_115915_, double p_115916_, double p_115917_) {
        return super.shouldRender((Mob)p_115913_, p_115914_, p_115915_, p_115916_, p_115917_) ? true : p_115913_.getRenderPosition(0.0F).filter((p_174374_) -> {
            ...
            return p_115914_.isVisible((new AABB(p_174374_.x, p_174374_.y + (double)$$4, p_174374_.z, $$6.x, $$6.y + (double)$$4, $$6.z)).inflate((double)$$5, (double)$$4, (double)$$5));
        }).isPresent();
    }
    ...
    static {
        ...
        TEXTURE_LOCATION = (ResourceLocation[])Sheets.SHULKER_TEXTURE_LOCATION.stream().map((p_115919_) -> {
