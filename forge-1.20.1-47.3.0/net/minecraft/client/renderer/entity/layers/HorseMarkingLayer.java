...
public class HorseMarkingLayer extends RenderLayer<Horse, HorseModel<Horse>> {
    private static final Map<Markings, ResourceLocation> LOCATION_BY_MARKINGS = (Map)Util.make(Maps.newEnumMap(Markings.class), (p_117069_) -> {
        p_117069_.put(Markings.NONE, (Object)null);
    ...
    public void render(PoseStack p_117058_, MultiBufferSource p_117059_, int p_117060_, Horse p_117061_, float p_117062_, float p_117063_, float p_117064_, float p_117065_, float p_117066_, float p_117067_) {
        ResourceLocation $$10 = (ResourceLocation)LOCATION_BY_MARKINGS.get(p_117061_.getMarkings());
        if ($$10 != null && !p_117061_.isInvisible()) {
            VertexConsumer $$11 = p_117059_.getBuffer(RenderType.entityTranslucent($$10));
            ((HorseModel)this.getParentModel()).renderToBuffer(p_117058_, $$11, p_117060_, LivingEntityRenderer.getOverlayCoords(p_117061_, 0.0F), 1.0F, 1.0F, 1.0F, 1.0F);
        }
    }
}
