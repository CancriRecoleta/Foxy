...
public class FaceBakery {
    ...
    public BakedQuad bakeQuad(Vector3f p_253895_, Vector3f p_253976_, BlockElementFace p_111603_, TextureAtlasSprite p_111604_, Direction p_111605_, ModelState p_111606_, @Nullable BlockElementRotation p_111607_, boolean p_111608_, ResourceLocation p_111609_) {
        ...
        ForgeHooksClient.fillNormal(aint, direction, p_111603_.getFaceData().calculateNormals());
        ForgeFaceData data = p_111603_.getFaceData();
        BakedQuad quad = new BakedQuad(aint, p_111603_.tintIndex, direction, p_111604_, p_111608_, data.ambientOcclusion());
    ...
    private float[] setupShape(Vector3f p_254153_, Vector3f p_253934_) {
        ...
        afloat[Constants.MIN_X] = p_254153_.x() / 16.0F;
        afloat[Constants.MIN_Y] = p_254153_.y() / 16.0F;
        afloat[Constants.MIN_Z] = p_254153_.z() / 16.0F;
        afloat[Constants.MAX_X] = p_253934_.x() / 16.0F;
        afloat[Constants.MAX_Y] = p_253934_.y() / 16.0F;
        afloat[Constants.MAX_Z] = p_253934_.z() / 16.0F;
        return afloat;
    }
    ...
    private void fillVertex(int[] p_111615_, int p_111616_, Vector3f p_254291_, TextureAtlasSprite p_111618_, BlockFaceUV p_111619_) {
        ...
        p_111615_[i + 3] = -1;
        p_111615_[i + 4] = Float.floatToRawIntBits(p_111618_.getU((double)p_111619_.getU(p_111616_) * 0.999 + (double)p_111619_.getU((p_111616_ + 2) % 4) * 0.001));
        p_111615_[i + 4 + 1] = Float.floatToRawIntBits(p_111618_.getV((double)p_111619_.getV(p_111616_) * 0.999 + (double)p_111619_.getV((p_111616_ + 2) % 4) * 0.001));
    }
    ...
    private void applyElementRotation(Vector3f p_254412_, @Nullable BlockElementRotation p_254150_) {
        if (p_254150_ != null) {
            ...
            Quaternionf quaternionf = (new Quaternionf()).rotationAxis(p_254150_.angle() * 0.017453292F, vector3f);
    ...
    private void recalculateWinding(int[] p_111631_, Direction p_111632_) {
        ...
        float[] afloat = new float[Direction.values().length];
        afloat[Constants.MIN_X] = 999.0F;
        afloat[Constants.MIN_Y] = 999.0F;
        afloat[Constants.MIN_Z] = 999.0F;
        afloat[Constants.MAX_X] = -999.0F;
        afloat[Constants.MAX_Y] = -999.0F;
        afloat[Constants.MAX_Z] = -999.0F;
        ...
        for(int i = 0; i < 4; ++i) {
            ...
            if (f < afloat[Constants.MIN_X]) {
                afloat[Constants.MIN_X] = f;
            }

            if (f1 < afloat[Constants.MIN_Y]) {
                afloat[Constants.MIN_Y] = f1;
            }

            if (f2 < afloat[Constants.MIN_Z]) {
                afloat[Constants.MIN_Z] = f2;
            }

            if (f > afloat[Constants.MAX_X]) {
                afloat[Constants.MAX_X] = f;
            }

            if (f1 > afloat[Constants.MAX_Y]) {
                afloat[Constants.MAX_Y] = f1;
            }

            if (f2 > afloat[Constants.MAX_Z]) {
                afloat[Constants.MAX_Z] = f2;
            }
        }
        ...
        for(i1 = 0; i1 < 4; ++i1) {
            ...
            for(int k = 0; k < 4; ++k) {
                ...
                if (Mth.equal(f2, f5) && Mth.equal(f3, f6) && Mth.equal(f4, f7)) {
