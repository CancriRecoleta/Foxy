...
public class BlockElement {
    ...
    private void fillUvs() {
        ...
        while(var1.hasNext()) {
            Map.Entry<Direction, BlockElementFace> entry = (Map.Entry)var1.next();
            float[] afloat = this.uvsByFace((Direction)entry.getKey());
            ((BlockElementFace)entry.getValue()).uv.setMissingUv(afloat);
        }
    ...
    public void setFaceData(ForgeFaceData faceData) {
        this.faceData = (ForgeFaceData)Objects.requireNonNull(faceData);
    }
    ...
    public static class Deserializer implements JsonDeserializer<BlockElement> {
        ...
        public BlockElement deserialize(JsonElement p_111329_, Type p_111330_, JsonDeserializationContext p_111331_) throws JsonParseException {
            ...
            if (jsonobject.has("shade") && !GsonHelper.isBooleanValue(jsonobject, "shade")) {
                ...
                ForgeFaceData faceData = ForgeFaceData.read(jsonobject.get("forge_data"), ForgeFaceData.DEFAULT);
        ...
        private Direction.Axis getAxis(JsonObject p_111345_) {
            ...
            Direction.Axis direction$axis = Axis.byName(s.toLowerCase(Locale.ROOT));
