...
public class LightTexture implements AutoCloseable {
    ...
    private float getDarknessGamma(float p_234320_) {
        if (this.minecraft.player.hasEffect(MobEffects.DARKNESS)) {
            ...
            if (mobeffectinstance != null && mobeffectinstance.getFactorData().isPresent()) {
                return ((MobEffectInstance.FactorData)mobeffectinstance.getFactorData().get()).getFactor(this.minecraft.player, p_234320_);
            }
        }
    ...
    private float calculateDarknessScale(LivingEntity p_234313_, float p_234314_, float p_234315_) {
        ...
        return Math.max(0.0F, Mth.cos(((float)p_234313_.tickCount - p_234315_) * 3.1415927F * 0.025F) * f);
    }
                    ...
                    for(int j = 0; j < 16; ++j) {
                        ...
                        if (flag) {
                            ...
                            if (this.renderer.getDarkenWorldAmount(p_109882_) > 0.0F) {
                                f15 = this.renderer.getDarkenWorldAmount(p_109882_);
                                vector3f5 = (new Vector3f(vector3f1)).mul(0.7F, 0.6F, 0.6F);
                        ...
                        if (f5 > 0.0F) {
                            ...
                            if (f14 < 1.0F) {
                                f15 = 1.0F / f14;
                                vector3f5 = (new Vector3f(vector3f1)).mul(f15);
                        ...
                        f14 = ((Double)this.minecraft.options.gamma().get()).floatValue();
