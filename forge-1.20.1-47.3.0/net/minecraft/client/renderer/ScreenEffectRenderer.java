...
public class ScreenEffectRenderer {
    ...
    public static void renderScreenEffect(Minecraft p_110719_, PoseStack p_110720_) {
        ...
        if (!player.noPhysics) {
            ...
            if (overlay != null && !ForgeHooksClient.renderBlockOverlay(player, p_110720_, OverlayType.BLOCK, (BlockState)overlay.getLeft(), (BlockPos)overlay.getRight())) {
                renderTex(p_110719_.getBlockRenderer().getBlockModelShaper().getTexture((BlockState)overlay.getLeft(), p_110719_.level, (BlockPos)overlay.getRight()), p_110720_);
            }
        }

        if (!p_110719_.player.isSpectator()) {
            if (p_110719_.player.isEyeInFluid(FluidTags.WATER)) {
                if (!ForgeHooksClient.renderWaterOverlay(player, p_110720_)) {
                    renderWater(p_110719_, p_110720_);
                }
            } else if (!player.getEyeInFluidType().isAir()) {
                IClientFluidTypeExtensions.of(player.getEyeInFluidType()).renderOverlay(p_110719_, p_110720_);
            }

            if (p_110719_.player.isOnFire() && !ForgeHooksClient.renderFireOverlay(player, p_110720_)) {
    ...
    private static BlockState getViewBlockingState(Player p_110717_) {
        return (BlockState)getOverlayBlock(p_110717_).getLeft();
    }
