...
public class LecternRenderer implements BlockEntityRenderer<LecternBlockEntity> {
    ...
    public void render(LecternBlockEntity p_112435_, float p_112436_, PoseStack p_112437_, MultiBufferSource p_112438_, int p_112439_, int p_112440_) {
        BlockState $$6 = p_112435_.getBlockState();
        if ((Boolean)$$6.getValue(LecternBlock.HAS_BOOK)) {
            p_112437_.pushPose();
            p_112437_.translate(0.5F, 1.0625F, 0.5F);
            float $$7 = ((Direction)$$6.getValue(LecternBlock.FACING)).getClockWise().toYRot();
            p_112437_.mulPose(Axis.YP.rotationDegrees(-$$7));
            p_112437_.mulPose(Axis.ZP.rotationDegrees(67.5F));
            p_112437_.translate(0.0F, -0.125F, 0.0F);
            this.bookModel.setupAnim(0.0F, 0.1F, 0.9F, 1.2F);
            VertexConsumer $$8 = EnchantTableRenderer.BOOK_LOCATION.buffer(p_112438_, RenderType::entitySolid);
            this.bookModel.render(p_112437_, $$8, p_112439_, p_112440_, 1.0F, 1.0F, 1.0F, 1.0F);
            p_112437_.popPose();
        }
    }
}
