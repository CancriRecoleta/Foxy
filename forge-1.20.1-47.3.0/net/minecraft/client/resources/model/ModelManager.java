...
public class ModelManager implements PreparableReloadListener, AutoCloseable {
    ...
    public BakedModel getModel(ModelResourceLocation p_119423_) {
        return (BakedModel)this.bakedRegistry.getOrDefault(p_119423_, this.missingModel);
    }
    ...
    public final CompletableFuture<Void> reload(PreparableReloadListener.PreparationBarrier p_249079_, ResourceManager p_251134_, ProfilerFiller p_250336_, ProfilerFiller p_252324_, Executor p_250550_, Executor p_249221_) {
        ...
        CompletableFuture var10000 = CompletableFuture.allOf((CompletableFuture[])Stream.concat(map.values().stream(), Stream.of(completablefuture2)).toArray((p_250157_) -> {
            return new CompletableFuture[p_250157_];
        })).thenApplyAsync((p_248624_) -> {
            return this.loadModels(p_250336_, (Map)map.entrySet().stream().collect(Collectors.toMap(Map.Entry::getKey, (p_248988_) -> {
                return (AtlasSet.StitchResult)((CompletableFuture)p_248988_.getValue()).join();
            })), (ModelBakery)completablefuture2.join());
        }, p_250550_).thenCompose((p_252255_) -> {
            return p_252255_.readyForUpload.thenApply((p_251581_) -> {
                return p_252255_;
            });
        });
        Objects.requireNonNull(p_249079_);
        return var10000.thenCompose(p_249079_::wait).thenAcceptAsync((p_252252_) -> {
    ...
    private static CompletableFuture<Map<ResourceLocation, BlockModel>> loadBlockModels(ResourceManager p_251361_, Executor p_252189_) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            Iterator var3 = p_250597_.entrySet().iterator();

            while(var3.hasNext()) {
                Map.Entry<ResourceLocation, Resource> entry = (Map.Entry)var3.next();
            ...
            return Util.sequence(list).thenApply((p_250813_) -> {
                return (Map)p_250813_.stream().filter(Objects::nonNull).collect(Collectors.toUnmodifiableMap(Pair::getFirst, Pair::getSecond));
            });
        });
    }
    ...
    private static CompletableFuture<Map<ResourceLocation, List<ModelBakery.LoadedJson>>> loadBlockStates(ResourceManager p_252084_, Executor p_249943_) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            Iterator var3 = p_250744_.entrySet().iterator();

            while(var3.hasNext()) {
                Map.Entry<ResourceLocation, List<Resource>> entry = (Map.Entry)var3.next();
                list.add(CompletableFuture.supplyAsync(() -> {
                    List<Resource> list1 = (List)entry.getValue();
                    ...
                    while(var3.hasNext()) {
                        ...
                        try {
                            ...
                            LOGGER.error("Failed to load blockstate {} from pack {}", new Object[]{entry.getKey(), resource.sourcePackId(), exception});
                        }
                    }
            ...
            return Util.sequence(list).thenApply((p_248966_) -> {
                return (Map)p_248966_.stream().filter(Objects::nonNull).collect(Collectors.toUnmodifiableMap(Pair::getFirst, Pair::getSecond));
            });
        });
    }
    ...
    private ReloadState loadModels(ProfilerFiller p_252136_, Map<ResourceLocation, AtlasSet.StitchResult> p_250646_, ModelBakery p_248945_) {
        ...
        multimap.asMap().forEach((p_250493_, p_252017_) -> {
            LOGGER.warn("Missing textures in model {}:\n{}", p_250493_, p_252017_.stream().sorted(Material.COMPARATOR).map((p_248692_) -> {
                ResourceLocation var10000 = p_248692_.atlasLocation();
                return "    " + var10000 + ":" + p_248692_.texture();
        ...
        BakedModel bakedmodel = (BakedModel)map.get(ModelBakery.MISSING_MODEL_LOCATION);
        ...
        while(var8.hasNext()) {
            ...
            block.getStateDefinition().getPossibleStates().forEach((p_250633_) -> {
                ...
                BakedModel bakedmodel1 = (BakedModel)map.getOrDefault(BlockModelShaper.stateToModelLocation(resourcelocation, p_250633_), bakedmodel);
                map1.put(p_250633_, bakedmodel1);
            });
        }

        CompletableFuture<Void> completablefuture = CompletableFuture.allOf((CompletableFuture[])p_250646_.values().stream().map(AtlasSet.StitchResult::readyForUpload).toArray((p_250044_) -> {
