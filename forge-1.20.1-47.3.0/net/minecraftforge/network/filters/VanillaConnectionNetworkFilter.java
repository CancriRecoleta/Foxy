...
public class VanillaConnectionNetworkFilter extends VanillaPacketFilter {
    ...
    private static @NotNull ClientboundUpdateAttributesPacket filterEntityProperties(ClientboundUpdateAttributesPacket msg) {
        ...
        msg.getValues().stream().filter((snapshot) -> {
            ...
            newPacket.getValues().add(snapshot);
        });
        return newPacket;
    }

    private static @NotNull ClientboundCommandsPacket filterCommandList(ClientboundCommandsPacket packet) {
    ...
    private static ClientboundUpdateTagsPacket filterCustomTagTypes(ClientboundUpdateTagsPacket packet) {
        Map<ResourceKey<? extends Registry<?>>, TagNetworkSerialization.NetworkPayload> tags = (Map)packet.getTags().entrySet().stream().filter((e) -> {
            return isVanillaRegistry(((ResourceKey)e.getKey()).location());
    ...
    private static boolean isVanillaRegistry(ResourceLocation location) {
        return RegistryManager.getVanillaRegistryKeys().contains(location) || VanillaRegistries.DATAPACK_REGISTRY_KEYS.stream().anyMatch((k) -> {
