...
public class VanillaPacketSplitter {
    ...
    public static void register() {
        ...
        EventNetworkChannel channel = NetworkRegistry.newEventChannel(CHANNEL, () -> {
            return "1.1";
    ...
    private static void onClientPacket(NetworkEvent.ServerCustomPayloadEvent event) {
        ...
        if (state == 2) {
            ...
            if (packet == null) {
                ...
                full.release();
                ctx.enqueueWork(() -> {
                    genericsFtw(packet, ((NetworkEvent.Context)event.getSource().get()).getNetworkManager().getPacketListener());
                });
            }
        }
    ...
    public static RemoteCompatibility getRemoteCompatibility(Connection manager) {
        ...
        return connectionData != null && connectionData.getChannels().containsKey(CHANNEL) ? net.minecraftforge.network.filters.VanillaPacketSplitter.RemoteCompatibility.PRESENT : net.minecraftforge.network.filters.VanillaPacketSplitter.RemoteCompatibility.ABSENT;
    }

    public static boolean isRemoteCompatible(Connection manager) {
        return getRemoteCompatibility(manager) != net.minecraftforge.network.filters.VanillaPacketSplitter.RemoteCompatibility.ABSENT;
    }
