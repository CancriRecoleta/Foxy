...
public class PlayMessages {
            ...
            ((NetworkEvent.Context)ctx.get()).enqueueWork(() -> {
                try {
                    MenuScreens.getScreenFactory(msg.getType(), Minecraft.getInstance(), msg.getWindowId(), msg.getName()).ifPresent((f) -> {
                        ...
                        Screen s = f.create(c, Minecraft.getInstance().player.getInventory(), msg.getName());
    ...
    public static class SpawnEntity {
        ...
        public static void encode(SpawnEntity msg, FriendlyByteBuf buf) {
            ...
            Entity var3 = msg.entity;
            if (var3 instanceof IEntityAdditionalSpawnData entityAdditionalSpawnData) {
        ...
        public static void handle(SpawnEntity msg, Supplier<NetworkEvent.Context> ctx) {
            ((NetworkEvent.Context)ctx.get()).enqueueWork(() -> {
                try {
                    ...
                    Optional<Level> world = (Optional)LogicalSidedProvider.CLIENTWORLD.get(((NetworkEvent.Context)ctx.get()).getDirection().getReceptionSide());
                    ...
                    world.filter(ClientLevel.class::isInstance).ifPresent((w) -> {
                        ((ClientLevel)w).putNonPlayerEntity(msg.entityId, e);
                    });
