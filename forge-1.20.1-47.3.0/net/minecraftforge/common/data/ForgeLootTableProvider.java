...
public final class ForgeLootTableProvider extends LootTableProvider {
    ...
    public List<LootTableProvider.SubProviderEntry> getTables() {
        return (List)super.getTables().stream().map((entry) -> {
            return new LootTableProvider.SubProviderEntry(() -> {
                return this.replaceAndFilterChangesOnly((LootTableSubProvider)entry.provider().get());
            }, entry.paramSet());
    ...
    private LootTableSubProvider replaceAndFilterChangesOnly(LootTableSubProvider subProvider) {
        return (newConsumer) -> {
            subProvider.generate((resourceLocation, builder) -> {
    ...
    private boolean findAndReplaceInLootTableBuilder(LootTable.Builder builder, Item from, ToolAction toolAction) {
        List<LootPool> lootPools = (List)ObfuscationReflectionHelper.getPrivateValue(LootTable.Builder.class, builder, "f_79156_");
        boolean found = false;
        if (lootPools == null) {
            throw new IllegalStateException(LootTable.Builder.class.getName() + " is missing field f_79156_");
        } else {
            Iterator var6 = lootPools.iterator();

            while(var6.hasNext()) {
                LootPool lootPool = (LootPool)var6.next();
    ...
    private boolean findAndReplaceInLootPool(LootPool lootPool, Item from, ToolAction toolAction) {
        ...
        if (lootEntries == null) {
            ...
            LootPoolEntryContainer[] var7 = lootEntries;
            int var8 = lootEntries.length;

            for(int var9 = 0; var9 < var8; ++var9) {
                LootPoolEntryContainer lootEntry = var7[var9];
    ...
    private boolean findAndReplaceInParentedLootEntry(CompositeEntryBase entry, Item from, ToolAction toolAction) {
        ...
        if (lootEntries == null) {
            ...
            LootPoolEntryContainer[] var6 = lootEntries;
            int var7 = lootEntries.length;

            for(int var8 = 0; var8 < var7; ++var8) {
                LootPoolEntryContainer lootEntry = var6[var8];
                if (this.findAndReplaceInLootEntry(lootEntry, from, toolAction)) {
                    found = true;
                }
            }
    ...
    private boolean findAndReplaceInLootEntry(LootPoolEntryContainer entry, Item from, ToolAction toolAction) {
        ...
        if (lootConditions == null) {
            ...
            for(int i = 0; i < lootConditions.length; ++i) {
                LootItemCondition var8 = lootConditions[i];
                if (var8 instanceof CompositeLootItemCondition) {
                    CompositeLootItemCondition composite = (CompositeLootItemCondition)var8;
                    if (this.findAndReplaceInComposite(composite, from, toolAction)) {
                        found = true;
                        continue;
                    }
                }
    ...
    private boolean checkMatchTool(MatchTool lootCondition, Item expected) {
        ...
        Set<Item> items = (Set)ObfuscationReflectionHelper.getPrivateValue(ItemPredicate.class, predicate, "f_151427_");
