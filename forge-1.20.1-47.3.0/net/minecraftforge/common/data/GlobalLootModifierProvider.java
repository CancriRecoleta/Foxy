...
public abstract class GlobalLootModifierProvider implements DataProvider {
    ...
    public CompletableFuture<?> run(CachedOutput cache) {
        this.start();
        Path forgePath = this.output.getOutputFolder(Target.DATA_PACK).resolve("forge").resolve("loot_modifiers").resolve("global_loot_modifiers.json");
        Path modifierFolderPath = this.output.getOutputFolder(Target.DATA_PACK).resolve(this.modid).resolve("loot_modifiers");
