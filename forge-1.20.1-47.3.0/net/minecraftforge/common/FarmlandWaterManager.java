...
public class FarmlandWaterManager {
    ...
    public static <T extends SimpleTicket<Vec3>> T addCustomTicket(Level level, T ticket, ChunkPos masterChunk, ChunkPos... additionalChunks) {
        ...
        Map<ChunkPos, ChunkTicketManager<Vec3>> ticketMap = (Map)customWaterHandler.computeIfAbsent(level, (id) -> {
    ...
    public static AABBTicket addAABBTicket(Level level, AABB aabb) {
        ...
        return (AABBTicket)addCustomTicket(level, new AABBTicket(aabb), masterPos, (ChunkPos[])posSet.toArray(new ChunkPos[0]));
    }
    ...
    public static boolean hasBlockWaterTicket(LevelReader level, BlockPos pos) {
        ...
        if (ticketManager != null) {
            Vec3 posAsVec3d = new Vec3((double)pos.getX() + 0.5, (double)pos.getY() + 0.5, (double)pos.getZ() + 0.5);
            Iterator var4 = ticketManager.getTickets().iterator();

            while(var4.hasNext()) {
                SimpleTicket<Vec3> ticket = (SimpleTicket)var4.next();
    ...
    private static ChunkTicketManager<Vec3> getTicketManager(ChunkPos pos, LevelReader level) {
        Preconditions.checkArgument(!level.isClientSide(), "Water region is only determined server-side");
        Map<ChunkPos, ChunkTicketManager<Vec3>> ticketMap = (Map)customWaterHandler.get(level);
