...
public final class TransformationHelper {
    ...
    public static Quaternionf slerp(Quaternionfc v0, Quaternionfc v1, float t) {
        float dot = v0.x() * ((Quaternionfc)v1).x() + v0.y() * ((Quaternionfc)v1).y() + v0.z() * ((Quaternionfc)v1).z() + v0.w() * ((Quaternionfc)v1).w();
        if (dot < 0.0F) {
            v1 = new Quaternionf(-((Quaternionfc)v1).x(), -((Quaternionfc)v1).y(), -((Quaternionfc)v1).z(), -((Quaternionfc)v1).w());
            dot = -dot;
        }
        ...
        if ((double)dot > 0.9995) {
            angle01 = Mth.lerp(t, v0.x(), ((Quaternionfc)v1).x());
            angle0t = Mth.lerp(t, v0.y(), ((Quaternionfc)v1).y());
            sin0t = Mth.lerp(t, v0.z(), ((Quaternionfc)v1).z());
            sin01 = Mth.lerp(t, v0.w(), ((Quaternionfc)v1).w());
            return new Quaternionf(angle01, angle0t, sin0t, sin01);
        } else {
            angle01 = (float)Math.acos((double)dot);
            angle0t = angle01 * t;
            sin0t = Mth.sin(angle0t);
            sin01 = Mth.sin(angle01);
            float sin1t = Mth.sin(angle01 - angle0t);
            float s1 = sin0t / sin01;
            float s0 = sin1t / sin01;
            return new Quaternionf(s0 * v0.x() + s1 * ((Quaternionfc)v1).x(), s0 * v0.y() + s1 * ((Quaternionfc)v1).y(), s0 * v0.z() + s1 * ((Quaternionfc)v1).z(), s0 * v0.w() + s1 * ((Quaternionfc)v1).w());
        }
    }

    public static Transformation slerp(Transformation one, Transformation that, float progress) {
        return new Transformation(lerp(one.getTranslation(), that.getTranslation(), progress), slerp((Quaternionfc)one.getLeftRotation(), (Quaternionfc)that.getLeftRotation(), progress), lerp(one.getScale(), that.getScale(), progress), slerp((Quaternionfc)one.getRightRotation(), (Quaternionfc)that.getRightRotation(), progress));
    }
    ...
    public static class Deserializer implements JsonDeserializer<Transformation> {
        ...
        public Transformation deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
            if (json.isJsonPrimitive() && json.getAsJsonPrimitive().isString()) {
                ...
                if (obj.has("matrix")) {
                    ...
                    Vector3f origin = net.minecraftforge.common.util.TransformationHelper.TransformOrigin.OPPOSING_CORNER.getVector();
        ...
        private static Vector3f parseOrigin(JsonObject obj) {
            ...
            if (originElement.isJsonArray()) {
                ...
                TransformOrigin originEnum = net.minecraftforge.common.util.TransformationHelper.TransformOrigin.fromString(originString);
                if (originEnum == null) {
            ...
            if (!e.isJsonArray()) {
                ...
                if (m.size() != 3) {
                    ...
                    for(int rowIdx = 0; rowIdx < 3; ++rowIdx) {
                        JsonElement var10002;
                        if (!m.get(rowIdx).isJsonArray()) {
                            var10002 = m.get(rowIdx);
                            throw new JsonParseException("Matrix row: expected an array, got: " + var10002);
                        }
                        ...
                        for(int columnIdx = 0; columnIdx < 4; ++columnIdx) {
                            try {
                                ...
                                var10002 = r.get(columnIdx);
                                throw new JsonParseException("Matrix element: expected number, got: " + var10002);
                            }
                        }
                    }
        ...
        public static Quaternionf parseAxisRotation(JsonElement e) {
            if (!e.isJsonObject()) {
                ...
                if (obj.entrySet().size() != 1) {
                    ...
                    Map.Entry<String, JsonElement> entry = (Map.Entry)obj.entrySet().iterator().next();
