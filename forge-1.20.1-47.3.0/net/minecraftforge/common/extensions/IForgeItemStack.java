...
public interface IForgeItemStack extends ICapabilitySerializable<CompoundTag> {
    ...
    default @Nullable CompoundTag getShareTag() {
    ...
    default boolean equals(ItemStack other, boolean limitTags) {
        if (this.self().isEmpty()) {
            ...
            boolean var10000;
            label35: {
                if (!other.isEmpty() && this.self().getCount() == other.getCount() && this.self().getItem() == other.getItem()) {
                    if (limitTags) {
                        if (this.self().areShareTagsEqual(other)) {
                            break label35;
                        }
                    } else if (Objects.equals(this.self().getTag(), other.getTag())) {
                        break label35;
                    }
                }
    ...
    default @NotNull AABB getSweepHitBox(@NotNull Player player, @NotNull Entity target) {
    ...
    default @Nullable FoodProperties getFoodProperties(@Nullable LivingEntity entity) {
