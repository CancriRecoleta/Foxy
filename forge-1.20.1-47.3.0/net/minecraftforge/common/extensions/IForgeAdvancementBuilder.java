...
public interface IForgeAdvancementBuilder {
    ...
    default Advancement save(Consumer<Advancement> saver, ResourceLocation id, ExistingFileHelper fileHelper) {
        boolean canBuild = this.self().canBuild((advancementId) -> {
            return fileHelper.exists(advancementId, PackType.SERVER_DATA, ".json", "advancements") ? new Advancement(advancementId, (Advancement)null, (DisplayInfo)null, AdvancementRewards.EMPTY, Maps.newHashMap(), new String[0][0], false) : null;
        });
        if (!canBuild) {
