...
public interface IForgeEntity extends ICapabilitySerializable<CompoundTag> {
    ...
    default @Nullable PartEntity<?>[] getParts() {
        return null;
    }

    default float getStepHeight() {
        float vanillaStep = this.self().maxUpStep();
        Entity var3 = this.self();
        if (var3 instanceof LivingEntity living) {
            AttributeInstance stepHeightAttribute = living.getAttribute((Attribute)ForgeMod.STEP_HEIGHT_ADDITION.get());
    ...
    default @Nullable SoundEvent getSoundFromFluidType(FluidType type, SoundAction action) {
