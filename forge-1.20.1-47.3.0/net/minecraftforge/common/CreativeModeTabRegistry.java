...
public final class CreativeModeTabRegistry {
    ...
    private static final Multimap<ResourceLocation, ResourceLocation> edges;
    ...
    public static @Nullable CreativeModeTab getTab(ResourceLocation name) {
        return (CreativeModeTab)BuiltInRegistries.CREATIVE_MODE_TAB.get(name);
    }

    public static @Nullable ResourceLocation getName(CreativeModeTab tab) {
    ...
    static PreparableReloadListener getReloadListener() {
        return new SimplePreparableReloadListener<JsonObject>() {
            ...
            protected @NotNull JsonObject prepare(@NotNull ResourceManager resourceManager, ProfilerFiller profiler) {
                Optional<Resource> res = resourceManager.getResource(CreativeModeTabRegistry.CREATIVE_MODE_TAB_ORDERING_JSON);
                if (res.isEmpty()) {
            ...
            protected void apply(@NotNull JsonObject data, @NotNull ResourceManager resourceManager, ProfilerFiller p) {
                try {
                    if (data.size() > 0) {
                        ...
                        List<CreativeModeTab> missingCreativeModeTabs = BuiltInRegistries.CREATIVE_MODE_TAB.stream().filter((CreativeModeTabx) -> {
                            return !customOrder.contains(CreativeModeTabx);
                        }).toList();
                        if (!missingCreativeModeTabs.isEmpty()) {
                            Stream var10002 = missingCreativeModeTabs.stream().map((CreativeModeTabx) -> {
                                return Objects.toString(CreativeModeTabRegistry.getName(CreativeModeTabx));
                            });
                            throw new IllegalStateException("CreativeModeTabs missing from the ordered list: " + (String)var10002.collect(Collectors.joining(", ")));
                        }
    ...
    private static void recalculateItemCreativeModeTabs() {
        ...
        Iterator var1 = BuiltInRegistries.CREATIVE_MODE_TAB.iterator();
        ...
        List<CreativeModeTab> tierList = TopologicalSort.topologicalSort(graph, Comparator.comparing((tabx) -> {
            return getName(tabx);
        }));
    ...
    public static void sortTabs() {
        ...
        Stream var10000 = BuiltInRegistries.CREATIVE_MODE_TAB.holders().filter((c) -> {
            return !DEFAULT_TABS.contains(c.get());
        });
        ...
        for(int i = 0; i < vanillaTabs; ++i) {
            Holder<CreativeModeTab> value = (Holder)indexed.get(i);
    ...
    private static void addTabOrder(CreativeModeTab tab, ResourceLocation name) {
        Iterator var2 = tab.tabsAfter.iterator();

        ResourceLocation before;
        while(var2.hasNext()) {
            before = (ResourceLocation)var2.next();
            edges.put(name, before);
        }

        var2 = tab.tabsBefore.iterator();
    ...
    static {
        SORTED_TABS_VIEW = Collections.unmodifiableList(SORTED_TABS);
        DEFAULT_TABS = new ArrayList();
