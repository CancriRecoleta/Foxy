...
public sealed class ChunkRenderTypeSet implements Iterable<RenderType> {
    ...
    public static ChunkRenderTypeSet of(Collection<RenderType> renderTypes) {
        return renderTypes.isEmpty() ? none() : of((Iterable)renderTypes);
    }
    ...
    private static ChunkRenderTypeSet of(Iterable<RenderType> renderTypes) {
        ...
        while(var2.hasNext()) {
            RenderType renderType = (RenderType)var2.next();
    ...
    public @NotNull Iterator<RenderType> iterator() {
    ...
    static {
        CHUNK_RENDER_TYPES = (RenderType[])CHUNK_RENDER_TYPES_LIST.toArray(new RenderType[0]);
        NONE = new None();
        ALL = new All();
    }
    ...
    private static final class All extends ChunkRenderTypeSet {
        private All() {
            super((BitSet)Util.make(new BitSet(), (bits) -> {
