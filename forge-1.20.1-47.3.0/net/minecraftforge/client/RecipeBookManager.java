...
public final class RecipeBookManager {
    ...
    public static <T extends Recipe<?>> @Nullable RecipeBookCategories findCategories(RecipeType<T> type, T recipe) {
        Function<Recipe<?>, RecipeBookCategories> lookup = (Function)RECIPE_CATEGORY_LOOKUPS.get(type);
    ...
    public static List<RecipeBookCategories> getCustomCategoriesOrEmpty(RecipeBookType recipeBookType) {
        return (List)TYPE_CATEGORIES.getOrDefault(recipeBookType, List.of());
    }
    ...
    static {
        AGGREGATE_CATEGORIES_VIEW = Collections.unmodifiableMap(AGGREGATE_CATEGORIES);
    }
}
