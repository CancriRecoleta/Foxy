        ...
        for(x = 0; x <= 2; ++x) {
            for(y = 0; y <= 2; ++y) {
                for(z = 0; z <= 2; ++z) {
                    ...
                    y1 = LevelRenderer.getLightColor(level, neighborState, this.pos);
                    this.s[x][y][z] = LightTexture.sky(y1);
                    this.b[x][y][z] = LightTexture.block(y1);
                    this.ao[x][y][z] = neighborState.getShadeBrightness(level, this.pos);
                }
            }
        }

        Direction[] var31 = SIDES;
        y = var31.length;

        int bxyz;
        int sxz;
        for(z = 0; z < y; ++z) {
            Direction side = var31[z];
            this.pos.setWithOffset(origin, (Direction)side);
            BlockState neighborState = level.getBlockState(this.pos);
            ...
            if (neighborState.getLightBlock(level, this.pos) == 15 || Shapes.faceShapeOccludes(thisStateShape.getFaceOcclusionShape(level, origin, side), otherStateShape.getFaceOcclusionShape(level, this.pos, side.getOpposite()))) {
                bxyz = side.getStepX() + 1;
                int y = side.getStepY() + 1;
                sxz = side.getStepZ() + 1;
                this.s[bxyz][y][sxz] = Math.max(this.s[1][1][1] - 1, this.s[bxyz][y][sxz]);
                this.b[bxyz][y][sxz] = Math.max(this.b[1][1][1] - 1, this.b[bxyz][y][sxz]);
            }
        }
