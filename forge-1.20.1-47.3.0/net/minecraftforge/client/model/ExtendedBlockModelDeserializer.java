...
public class ExtendedBlockModelDeserializer extends BlockModel.Deserializer {
    ...
    public ExtendedBlockModelDeserializer() {
    }
    ...
    public BlockModel deserialize(JsonElement element, Type targetType, JsonDeserializationContext deserializationContext) throws JsonParseException {
        ...
        if (jsonobject.has("visibility")) {
            ...
            Iterator var9 = visibility.entrySet().iterator();

            while(var9.hasNext()) {
                Map.Entry<String, JsonElement> part = (Map.Entry)var9.next();
    ...
    public static @Nullable IUnbakedGeometry<?> deserializeGeometry(JsonDeserializationContext deserializationContext, JsonObject object) throws JsonParseException {
        if (!object.has("loader")) {
            return null;
        } else {
            ResourceLocation name = new ResourceLocation(GsonHelper.getAsString(object, "loader"));
            IGeometryLoader<? extends IUnbakedGeometry<?>> loader = GeometryLoaderManager.get(name);
            if (loader == null) {
