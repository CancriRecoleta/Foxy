...
public class UnbakedGeometryHelper {
    ...
    public static List<BlockElement> createUnbakedItemElements(int layerIndex, SpriteContents spriteContents) {
        return createUnbakedItemElements(layerIndex, spriteContents, (ForgeFaceData)null);
    }
    ...
    public static List<BlockElement> createUnbakedItemMaskElements(int layerIndex, SpriteContents spriteContents) {
        return createUnbakedItemMaskElements(layerIndex, spriteContents, (ForgeFaceData)null);
    }
                    ...
                    if (xStart == -1) {
                        ...
                        elements.add(new BlockElement(new Vector3f((float)(16 * xStart) / (float)width, 16.0F - (float)(16 * yEnd) / (float)height, 7.5F), new Vector3f((float)(16 * x) / (float)width, 16.0F - (float)(16 * y) / (float)height, 8.5F), (Map)Util.make(new HashMap(), (map) -> {
                            ...
                            for(int var4 = 0; var4 < var3; ++var4) {
                                ...
                                map.put(direction, new BlockElementFace((Direction)null, layerIndex, "layer" + layerIndex, new BlockFaceUV((float[])null, 0)));
                            }
    ...
    public static void bakeElements(IModelBuilder<?> builder, List<BlockElement> elements, Function<Material, TextureAtlasSprite> spriteGetter, ModelState modelState, ResourceLocation modelLocation) {
        ...
        while(var5.hasNext()) {
            BlockElement element = (BlockElement)var5.next();
