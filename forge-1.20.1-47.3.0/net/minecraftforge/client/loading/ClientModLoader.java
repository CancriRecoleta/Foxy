...
public class ClientModLoader {
    ...
    public static void begin(Minecraft minecraft, PackRepository defaultResourcePacks, ReloadableResourceManager mcResourceManager) {
        ...
        if (error == null) {
            ...
            PackType var10003 = PackType.CLIENT_RESOURCES;
            Objects.requireNonNull(defaultResourcePacks);
            var10000.postEvent(new AddPackFindersEvent(var10003, defaultResourcePacks::addPackFinder));
    ...
    private static CompletableFuture<Void> onResourceReload(PreparableReloadListener.PreparationBarrier stage, ResourceManager resourceManager, ProfilerFiller prepareProfiler, ProfilerFiller executeProfiler, Executor asyncExecutor, Executor syncExecutor) {
        CompletableFuture var10000 = CompletableFuture.runAsync(createRunnableWithCatch(() -> {
            startModLoading(ModWorkManager.wrappedExecutor(syncExecutor), asyncExecutor);
        }), ModWorkManager.parallelExecutor());
        Objects.requireNonNull(stage);
        return var10000.thenCompose(stage::wait).thenRunAsync(() -> {
            finishModLoading(ModWorkManager.wrappedExecutor(syncExecutor), asyncExecutor);
        }, ModWorkManager.parallelExecutor());
    }
    ...
    private static void startModLoading(ModWorkManager.DrivenExecutor syncExecutor, Executor parallelExecutor) {
        createRunnableWithCatch(() -> {
            ModLoader.get().loadMods(syncExecutor, parallelExecutor, ImmediateWindowHandler::renderTick);
        }).run();
    }

    private static void finishModLoading(ModWorkManager.DrivenExecutor syncExecutor, Executor parallelExecutor) {
        createRunnableWithCatch(() -> {
            ModLoader.get().finishMods(syncExecutor, parallelExecutor, ImmediateWindowHandler::renderTick);
        }).run();
        loading = false;
    ...
    public static VersionChecker.Status checkForUpdates() {
        boolean anyOutdated = ModList.get().getMods().stream().map(VersionChecker::getResult).map((result) -> {
            return result.status();
        }).anyMatch((status) -> {
            return status == Status.OUTDATED || status == Status.BETA_OUTDATED;
        });
        ...
        if (!showWarnings) {
            if (!warnings.isEmpty()) {
                ...
                warnings.forEach((warning) -> {
                    LOGGER.warn(Logging.LOADING, warning.formatToString());
                });
            }
    ...
    private static RepositorySource buildPackFinder(Map<IModFile, ? extends PathPackResources> modResourcePacks) {
        return (packAcceptor) -> {
            clientPackFinder(modResourcePacks, packAcceptor);
        };
    }
    ...
    private static void clientPackFinder(Map<IModFile, ? extends PathPackResources> modResourcePacks, Consumer<Pack> packAcceptor) {
        ...
        Iterator var3 = modResourcePacks.entrySet().iterator();

        while(var3.hasNext()) {
            Map.Entry<IModFile, ? extends PathPackResources> e = (Map.Entry)var3.next();
            IModInfo mod = (IModInfo)((IModFile)e.getKey()).getModInfos().get(0);
            String name = "mod:" + mod.getModId();
            Pack modPack = Pack.readMetaAndCreate(name, Component.literal(((PathPackResources)e.getValue()).packId()), false, (id) -> {
                return (PackResources)e.getValue();
            }, PackType.CLIENT_RESOURCES, Position.BOTTOM, PackSource.DEFAULT);
            if (modPack == null) {
        ...
        Pack modResourcesPack = Pack.readMetaAndCreate("mod_resources", Component.literal("Mod Resources"), true, (id) -> {
            return new DelegatingPackResources(id, false, new PackMetadataSection(Component.translatable("fml.resources.modresources", hiddenPacks.size()), SharedConstants.getCurrentVersion().getPackVersion(PackType.CLIENT_RESOURCES)), hiddenPacks);
        }, PackType.CLIENT_RESOURCES, Position.BOTTOM, PackSource.DEFAULT);
