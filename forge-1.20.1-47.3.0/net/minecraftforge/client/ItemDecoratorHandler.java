...
public final class ItemDecoratorHandler {
    ...
    public static void init() {
        ...
        ImmutableMap.Builder<Item, ItemDecoratorHandler> builder = new ImmutableMap.Builder();
        decorators.forEach((item, itemDecorators) -> {
            builder.put(item, new ItemDecoratorHandler(itemDecorators));
        });
    ...
    public void render(GuiGraphics guiGraphics, Font font, ItemStack stack, int xOffset, int yOffset) {
        ...
        Iterator var6 = this.itemDecorators.iterator();

        while(var6.hasNext()) {
            IItemDecorator itemDecorator = (IItemDecorator)var6.next();
