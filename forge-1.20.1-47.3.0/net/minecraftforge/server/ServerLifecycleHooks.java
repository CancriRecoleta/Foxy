...
public class ServerLifecycleHooks {
    ...
    public static RepositorySource buildPackFinder(Map<IModFile, ? extends PathPackResources> modResourcePacks) {
        return (packAcceptor) -> {
            serverPackFinder(modResourcePacks, packAcceptor);
        };
    }

    private static void serverPackFinder(Map<IModFile, ? extends PathPackResources> modResourcePacks, Consumer<Pack> packAcceptor) {
        Iterator var2 = modResourcePacks.entrySet().iterator();

        while(var2.hasNext()) {
            Map.Entry<IModFile, ? extends PathPackResources> e = (Map.Entry)var2.next();
            ...
            if (!Objects.equals(mod.getModId(), "minecraft")) {
                ...
                }, PackType.SERVER_DATA, Position.BOTTOM, PackSource.DEFAULT);
                if (modPack == null) {
    ...
    private static void runModifiers(MinecraftServer server) {
        ...
        List<BiomeModifier> biomeModifiers = registries.registryOrThrow(Keys.BIOME_MODIFIERS).holders().map(Holder::value).toList();
        List<StructureModifier> structureModifiers = registries.registryOrThrow(Keys.STRUCTURE_MODIFIERS).holders().map(Holder::value).toList();
        registries.registryOrThrow(Registries.BIOME).holders().forEach((biomeHolder) -> {
            ((Biome)biomeHolder.value()).modifiableBiomeInfo().applyBiomeModifiers(biomeHolder, biomeModifiers);
        });
        registries.registryOrThrow(Registries.STRUCTURE).holders().forEach((structureHolder) -> {
            ((Structure)structureHolder.value()).modifiableStructureInfo().applyStructureModifiers(structureHolder, structureModifiers);
        });
    }
}
