...
class EntityCommand {
    ...
    static ArgumentBuilder<CommandSourceStack, ?> register() {
        return Commands.literal("entity").then(net.minecraftforge.server.command.EntityCommand.EntityListCommand.register());
    }
    ...
    private static class EntityListCommand {
        ...
        private static final DynamicCommandExceptionType INVALID_DIMENSION = new DynamicCommandExceptionType((dim) -> {
            return Component.translatable("commands.forge.entity.list.invalidworld", dim);
        });
                ...
                if (level == null) {
                    ...
                    if (names.size() != 1) {
                        ...
                        if (info.size() == 0) {
                            ...
                            sender.sendSuccess(() -> {
                                return Component.translatable("commands.forge.entity.list.multiple.header", count);
                            }, false);
                            info.forEach((ex) -> {
                                sender.sendSuccess(() -> {
                        ...
                        if (info == null) {
                            ...
                            sender.sendSuccess(() -> {
                                return Component.translatable("commands.forge.entity.list.single.header", name, info.getLeft());
                            }, false);
