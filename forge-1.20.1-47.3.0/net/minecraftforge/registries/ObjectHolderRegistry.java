...
public class ObjectHolderRegistry {
    ...
    public static void findObjectHolders() {
        ...
        List<ModFileScanData.AnnotationData> annotations = ModList.get().getAllScanData().stream().map(ModFileScanData::getAnnotations).flatMap(Collection::stream).filter((a) -> {
            return OBJECT_HOLDER.equals(a.annotationType()) || MOD.equals(a.annotationType());
        }).toList();
        Map<Type, String> classModIds = Maps.newHashMap();
        Map<Type, Class<?>> classCache = Maps.newHashMap();
        annotations.stream().filter((a) -> {
            return MOD.equals(a.annotationType());
        }).forEach((data) -> {
            classModIds.put(data.clazz(), (String)data.annotationData().get("value"));
        });
        VANILLA_OBJECT_HOLDERS.forEach((data) -> {
            try {
                ...
                scanTarget(classModIds, classCache, holderType, (String)null, registryClass, data.registryName(), "minecraft", true, true);
        ...
        annotations.stream().filter((a) -> {
            return OBJECT_HOLDER.equals(a.annotationType());
        }).filter((a) -> {
            return a.targetType() == ElementType.FIELD;
        }).forEach((data) -> {
            scanTarget(classModIds, classCache, data.clazz(), data.memberName(), (Class)null, (String)data.annotationData().get("registryName"), (String)data.annotationData().get("value"), false, false);
        });
    ...
    private static void scanTarget(Map<Type, String> classModIds, Map<Type, Class<?>> classCache, Type type, @Nullable String annotationTarget, @Nullable Class<?> registryClass, String registryName, String value, boolean isClass, boolean extractFromValue) {
        ...
        if (classCache.containsKey(type)) {
            ...
            try {
                ...
                ClassNotFoundException ex = var13;
    ...
    private static record VanillaObjectHolderData(String holderClass, String registryName, String registryType) {
        private VanillaObjectHolderData(String holderClass, String registryName, String registryType) {
