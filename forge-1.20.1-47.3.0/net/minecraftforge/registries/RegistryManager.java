...
public class RegistryManager {
    ...
    <V> ForgeRegistry<V> createRegistry(ResourceLocation name, RegistryBuilder<V> builder) {
        if (this.registries.containsKey(name)) {
            ...
            Iterator var4 = builder.getLegacyNames().iterator();

            while(var4.hasNext()) {
                ResourceLocation legacyName = (ResourceLocation)var4.next();
    ...
    private static <V> void injectForgeRegistry(ForgeRegistry<V> forgeReg, Registry<? extends Registry<?>> rootRegistry) {
        WritableRegistry<Registry<V>> registry = (WritableRegistry)rootRegistry;
    ...
    public Map<ResourceLocation, ForgeRegistry.Snapshot> takeSnapshot(boolean savingToDisc) {
        ...
        keys.forEach((name) -> {
            ret.put(name, this.getRegistry(name).makeSnapshot());
        });
        return ret;
    }
    ...
    public static List<Pair<String, HandshakeMessages.S2CRegistry>> generateRegistryPackets(boolean isLocal) {
        return !isLocal ? (List)ACTIVE.takeSnapshot(false).entrySet().stream().map((e) -> {
            return Pair.of("Registry " + e.getKey(), new HandshakeMessages.S2CRegistry((ResourceLocation)e.getKey(), (ForgeRegistry.Snapshot)e.getValue()));
