...
public class NotHolderSet<T> implements ICustomHolderSet<T> {
    ...
    public static <T> Codec<? extends ICustomHolderSet<T>> codec(ResourceKey<? extends Registry<T>> registryKey, Codec<Holder<T>> holderCodec, boolean forceList) {
        return RecordCodecBuilder.create((builder) -> {
            return builder.group(RegistryOps.retrieveRegistryLookup(registryKey).forGetter(NotHolderSet::registryLookup), HolderSetCodec.create(registryKey, holderCodec, forceList).fieldOf("value").forGetter(NotHolderSet::value)).apply(builder, NotHolderSet::new);
        });
    }
    ...
    public HolderSetType type() {
        return (HolderSetType)ForgeMod.NOT_HOLDER_SET.get();
    }
    ...
    private List<Holder<T>> getList() {
        ...
        if (thisList == null) {
            List<Holder<T>> list = this.registryLookup.listElements().filter((holder) -> {
                return !this.value.contains(holder);
            }).map((holder) -> {
                return holder;
            }).toList();
