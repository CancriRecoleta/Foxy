...
public interface IForgeRegistry<V> extends Iterable<V> {
    ...
    @Nullable V getValue(ResourceLocation var1);

    @Nullable ResourceLocation getKey(V var1);

    @Nullable ResourceLocation getDefaultKey();

    @NotNull Optional<ResourceKey<V>> getResourceKey(V var1);

    @NotNull Set<ResourceLocation> getKeys();

    @NotNull Collection<V> getValues();

    @NotNull Set<Map.Entry<ResourceKey<V>, V>> getEntries();

    @NotNull Codec<V> getCodec();

    @NotNull Optional<Holder<V>> getHolder(ResourceKey<V> var1);

    @NotNull Optional<Holder<V>> getHolder(ResourceLocation var1);

    @NotNull Optional<Holder<V>> getHolder(V var1);

    @Nullable ITagManager<V> tags();

    @NotNull Optional<Holder.Reference<V>> getDelegate(ResourceKey<V> var1);

    @NotNull
    Holder.@NotNull Reference<V> getDelegateOrThrow(ResourceKey<V> var1);

    @NotNull Optional<Holder.Reference<V>> getDelegate(ResourceLocation var1);

    @NotNull
    Holder.@NotNull Reference<V> getDelegateOrThrow(ResourceLocation var1);

    @NotNull Optional<Holder.Reference<V>> getDelegate(V var1);

    @NotNull
    Holder.@NotNull Reference<V> getDelegateOrThrow(V var1);
